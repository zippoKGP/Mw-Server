// NumberIndex.cpp: implementation of the CNumberIndex class.
//
//////////////////////////////////////////////////////////////////////
...
UG_ULONG CNumberIndex::init(UG_PCHAR pchPathName,UG_BOOL bLoadInMem)
{
	...
	UG_PCHAR pchKey = new UG_CHAR[fl.m_ulMaxLen + 1];
	...
	for(it = fl.m_list.begin(); it != fl.m_list.end(); it ++)
	{
		if(((*it).ulLineCount))
		{
			...
			*(pchLine + ulRead) = '\0';
			parseKeyAndValue(pchLine,pchKey,(*it).ulFilePos);
		}
	}
	if(m_bLoadInMem)
	{
...
UG_ULONG CNumberIndex::getValue(UG_ULONG ulKey,UG_PCHAR& pchValue)
{
	map<UG_ULONG,CIndexKeyValue*>::iterator it = m_map.find(ulKey);
	if(m_map.end() == it)
	{
		return -1;
	}
...
UG_ULONG CNumberIndex::cleanup()
{
	...
	map<UG_ULONG,CIndexKeyValue*>::iterator it;
...
UG_ULONG CNumberIndex::parseKeyAndValue(UG_PCHAR pchLine,UG_PCHAR pchKey,UG_ULONG ulPos)
{
...
UG_ULONG CNumberIndex::removeSpace(UG_PCHAR pchLine,UG_INT32 n32Count,UG_INT32& n32Left,UG_INT32& n32Right)
{
	n32Left = 0;
	n32Right = n32Count;
