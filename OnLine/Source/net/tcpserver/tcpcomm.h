...
public: //Server
	...
	UG_ULONG		sendBuffer(UG_PVOID pvConnect,UG_PVOID pvFlag); //发送缓冲中的数据
...
public: //Client
	...
	//写到发送缓冲中，不发数据，缓冲数据大小为ulSendSocketBufferSize，返回0正确,否则错误
	UG_ULONG		writeBuffer(UG_PCHAR pchBuffer,UG_ULONG ulLen); //写到发送缓冲中，不发数据，缓冲数据大小为ulSendSocketBufferSize
	//直接将数据发送出去，返回0正确，-1错误，该连接应该关闭，否则表示以发送的数据量，也是错误，该也连接应该关闭
	UG_ULONG		sendData(UG_PCHAR pchBuffer,UG_ULONG ulLen,UG_PVOID pvFlag); //直接将数据发送出去
	//发送缓冲数据，返回0正确，-1错误，该连接应该关闭，否则表示以发送的数据量，也是错误，该也连接应该关闭
	UG_ULONG		sendBuffer(UG_PVOID pvFlag); //发送缓冲中的数据
	//接收缓冲数据，返回0正确，-1错误，该连接应该关闭，否则表示以发送的数据量
	UG_ULONG		recvData(UG_PVOID pvFlag); //pvFlag为回调函数传出的指针tp.pvFlag
