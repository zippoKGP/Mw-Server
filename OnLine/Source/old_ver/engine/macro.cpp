...
void mc_exec_popshop_push(void)
{
	...
	if(character_pocket_data.idx>=0)
	{
		if((character_pocket_data.item[store_data.idx].item_id>=0)&&(character_pocket_data.item[store_data.idx].filename_id>=0))		// 单价
		{
// 当铺 存取 不用钱			
//			sprintf((char *)print_rec,"%d",character_pocket_data.item[character_pocket_data.idx].single_price);
...
void mc_exec_popshop_pop(void)
{
	...
	{
		if((character_popshop_data.item[store_data.idx].item_id>=0)&&(character_popshop_data.item[store_data.idx].filename_id>=0))		// 单价
		{
// 当铺不用钱
