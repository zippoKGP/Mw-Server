...
// --- ( 1 - 0 ) Cehck 	In Border 
	...
	{
		if((left_border_x<=MOVE_X_BORDER+100)||(right_border_x<=MOVE_X_BORDER+100))
			x_move_offset=3;
		else
			x_move_offset=1;
	}

	if((up_border_y<=MOVE_Y_BORDER)||(down_border_y<=MOVE_Y_BORDER))
	{
		if(map_npc_group[system_control.control_npc_idx].npc_info.speed==0)	// 走路
			y_move_offset=STAND_MOVE_STEP;
		else
	...
	{
		if((up_border_y<=MOVE_Y_BORDER+75)||(down_border_y<=MOVE_Y_BORDER+75))
			y_move_offset=3;
		else
			y_move_offset=1;
...
// --- ( 0 - 2 ) Copy to temp
	for(i=0,j=0;i<(ULONG)total_map_npc;i++)
	{
		if(i==(ULONG)map_npc_group_index)
		{
			continue;
		}
...
// --- ( 2 - 1 ) Copy temp to mda_group
	...
	for(i=0;i<(ULONG)total_map_npc;i++)
		memcpy(&map_npc_group[i],&temp_map_npc_group[i],sizeof(struct MAP_NPC_GROUP_STRUCT));
...
void display_chat_inter_window(BMP *buffer)
{
	...
	while(main_pass==0)
	{
		...
		case CHAT_CHANNEL_SYSTEM:			// 系统说话
			show_name=false;
			break;
		case CHAT_CHANNEL_MESSGAE:			// 随机讯息
			show_name=false;
		default:
			show_name=false;
			break;
