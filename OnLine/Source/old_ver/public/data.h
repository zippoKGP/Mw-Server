...
#define MAX_DETAIL_INST_LENGTH						128		// 详细说明长度
...
*/
	{	"衰老术一阶",     	"单体衰老",    	     4,	     5,	       0.080,	     3,	     1,	     1,	     1,	    201,	 201 },
...
// --- 基本攻击部分
	...
	FIGHT_ACTION_SINGLE_SUCK=						7,		// 吸取单人
	FIGHT_ACTION_SINGLE_VIRUS=						8,		// 中毒单人
	FIGHT_ACTION_SINGLE_STATUS=						9,		// 状态单人(石化,混,睡,死亡宣告.... )




// ---- 法术技能部分
	FIGHT_ACTION_MULTI_PHYSICS=						100,	// 物理多攻击 ( 含 feedback,致命, 狂暴 )
	FIGHT_ACTION_MULTI_PHYSICS_DOUBLE=				101,	// 物理多连击 ( 含 feedback, 致命，狂暴 )

	FIGHT_ACTION_MULTI_MAGIC=						102,	// 法术多攻击 ( 含 feedback, 狂暴 )
	FIGHT_ACTION_MULTI_MAGIC_DOUBLE=				103,	// 法术连击多攻击 ( 含 feedback, 狂暴 )

	FIGHT_ACTION_MULIT_SUCK=						104,	// 吸取多人
	FIGHT_ACTION_MULTI_VIRUS=						105,	// 中毒多人
	FIGHT_ACTION_MULTI_STATUS=						106,	// 状态多人(石化,混,睡,死亡宣告.... )
...
typedef struct FIGHT_ACTION_RESULE_STRUCT
{
	...
	ULONG target_id;								// 受动作人 id 如果有定义的动作,此参数无效
	char idx;										// 其他资料 ( 法术编号...等等)
...
typedef struct FIGHT_ACTION_SINGLE_SUCK_DATA_STRUCT
{
	SLONG hp;										// 吸取血量
	SLONG mp;										// 吸取法量
...
typedef struct FIGHT_ACTION_SINGLE_VIRUS_DATA_STRUCT
{
	SLONG hp;										// 毒血量
...
// ---- 多人技能部分
typedef struct FIGHT_ACTION_MULTI_PHYSICS_DATA_STRUCT
{													// 物理多攻击 ( 含 feedback,致命, 狂暴 )
	char number;									// 攻击数量
...
typedef struct FIGHT_ACTION_MULTI_PHYSICS_DOUBLE_DATA_STRUCT
{													// 物理多连击 ( 含 feedback, 致命，狂暴 )
	char number;									// 攻击数量
	char double_count;								// 连击次数
	ULONG id[MAX_FIGHT_UNIT];						// 被攻击方id
...
typedef struct FIGHT_ACTION_MULIT_SUCK_DATA_STRUCT
{													// 吸取多人
	char number;									// 吸取数量
	ULONG id[MAX_FIGHT_UNIT];						// 被攻击方id
	SLONG hp[MAX_FIGHT_UNIT];						// 吸取血量
	SLONG mp[MAX_FIGHT_UNIT];						// 吸取法量
...
typedef struct FIGHT_ACTION_MULTI_VIRUS_DATA_STRUCT
{
	char number;									// 中毒多人
	ULONG id[MAX_FIGHT_UNIT];						// 被攻击方id
	SLONG hp[MAX_FIGHT_UNIT];						// 毒血量
	SLONG loop_hp[MAX_FIGHT_UNIT];					// 这回合掉血量
	
} FIGHT_ACTION_MULTI_VIRUS_DATA;



typedef struct FIGHT_ACTION_MULTI_STATUS_DATA_STRUCT
{													// 状态多人(石化,混,睡,死亡宣告..		
	ULONG id[MAX_FIGHT_UNIT];						// 被攻击方id
