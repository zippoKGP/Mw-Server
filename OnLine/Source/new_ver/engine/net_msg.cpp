										 ...
										 const UG_PCHAR pchData,UG_ULONG ulLen)
{
	...
	if(NET_MSG_PLAYER_WEATHER_RETURN == dwMsgID)
	{
		net_lock = false;
		return 0;
	}
	...
	case NET_MSG_PLAYER_EQUIPE_CHANGE_RETURN:
		{
			...
			case EQUIPE_CHANGE_ERROR_PHYLE:
				{
					display_error_message((UCHR*)"种族不符合", 1);
				}
				break;

			case EQUIPE_CHANGE_ERROR_LEVEL:
				{
					display_error_message((UCHR*)"等级不够", 1);
				}
				break;

			case EQUIPE_CHANGE_ERROR_HP:
				{
					display_error_message((UCHR*)"HP不够", 1);
				}
				break;

			case EQUIPE_CHANGE_ERROR_MP:
				{
					display_error_message((UCHR*)"MP不够", 1);
				}
				break;

			case EQUIPE_CHANGE_ERROR_ATT:
				{
					display_error_message((UCHR*)"ATT不够", 1);
				}
				break;

			case EQUIPE_CHANGE_ERROR_SP:
				{
					display_error_message((UCHR*)"SP不够", 1);
				}
				break;
	...
	case NET_MSG_PLAYER_FIGHT_RESULT_RETURN:
		{
			if (pchData)
			{
				...
				result.value = (FIGHT_VALUE_RESULT*)(pchData+sizeof(result.base));

				result.cust_buf = pchData+sizeof(result.base)+result.base.value_num*sizeof(*result.value);
				...
				case FIGHT_COMMAND_ATTACK:
					{
						...
						if (loop == 1)
						{
							...
							fight_action_single_physics_data.protect_id = *hp ++;
						}
						else
						{
				...
				case FIGHT_COMMAND_CHANGE:
					{
						...
						uint* id_list = (uint*)result.cust_buf;
						...
						for (int i = 0; i < num; i ++)
						{
							...
							{
								if (fight_npc_group[j].base.id == id)
								{
									fight_npc_group[j].base.charge_speed = i;
