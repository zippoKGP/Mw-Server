...
#ifdef PFN_VECTORMA
void _SSE_VectorMA(const float* start, float scale, const float* direction,
	float* dest);
#endif
...
void FASTCALL _VectorNormalizeFast(Vector& vec)
{
	...
	// FLT_EPSILON is added to the radius to eliminate the possibility of divide by zero.
	float iradius = 1.f /
		(sqrtf(vec.x* vec.x + vec.y* vec.y + vec.z* vec.z) + FLT_EPSILON);
...
{
	float ool = _SSE_RSqrtAccurate(FLT_EPSILON +
					vec.x* vec.x +
					vec.y* vec.y +
					vec.z* vec.z);
...
// fast axial cases
	...
	{
	case 0:
		dist1 = p->normal[0] * emaxs[0] +
			p->normal[1] * emaxs[1] +
			p->normal[2] * emaxs[2];
		dist2 = p->normal[0] * emins[0] +
			p->normal[1] * emins[1] +
			p->normal[2] * emins[2];
		break;
	case 1:
		dist1 = p->normal[0] * emins[0] +
			p->normal[1] * emaxs[1] +
			p->normal[2] * emaxs[2];
		dist2 = p->normal[0] * emaxs[0] +
			p->normal[1] * emins[1] +
			p->normal[2] * emins[2];
		break;
	case 2:
...
void VectorMA(const float* start, float scale, const float* direction,
	float* dest)
#else
void _VectorMA(const float* start, float scale, const float* direction,
	float* dest)
#endif
{
	Assert(s_bMathlibInitialized);
...
_SSE_VectorMA(const float* start, float scale, const float* direction,
	float* dest)
{
...
__declspec( naked ) int BoxOnPlaneSide(const float* emins, const float* emaxs,
	const cplane_t* p)
{
	Assert(s_bMathlibInitialized);
...
_declspec(naked)
#if PFN_VECTORMA
void _VectorMA(const Vector& start, float scale, const Vector& direction,
	Vector& dest)
#else
...
#if PFN_VECTORMA
void _declspec(naked)
_SSE_VectorMA(const Vector& start, float scale, const Vector& direction,
	Vector& dest)
{
	// FIXME: This don't work!! It will overwrite memory in the write to dest
	Assert(0);
...
void CalcClosestPointOnLine(const Vector& P, const Vector& vLineA,
	const Vector& vLineB, Vector& vClosest, float* outT)
{
	assert(s_bMathlibInitialized);
	Vector vDir;
...
float CalcClosestPointToLineT2D(Vector2D const & P, Vector2D const & vLineA,
	Vector2D const & vLineB, Vector2D& vDir)
{
	Assert(s_bMathlibInitialized);
...
void CalcClosestPointOnLine2D(Vector2D const & P, Vector2D const & vLineA,
	Vector2D const & vLineB, Vector2D& vClosest, float* outT)
{
	assert(s_bMathlibInitialized);
	Vector2D vDir;
...
float CalcDistanceToLine2D(Vector2D const & P, Vector2D const & vLineA,
	Vector2D const & vLineB, float* outT)
{
	Assert(s_bMathlibInitialized);
	Vector2D vClosest;
...
void MathLib_Init(float gamma, float texGamma, float brightness, int overbright,
	bool bAllow3DNow, bool bAllowSSE, bool bAllowSSE2, bool bAllowMMX)
{
...
*/
// ============================================================================
void QuatMul(quat_t q1, quat_t q2, quat_t res)
{
...
void QuatToAngleAxis(quat_t quat, angleaxis_t angleaxis)
{
	...
	angleaxis[W] = (tw * 180.f) / (float) M_PI;
