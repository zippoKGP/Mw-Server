...
{ MAX_POLY_NUM              = 15,   // 轮廓最多顶点数
RIGHTFUL_POLY_MIN_NUM       = 3,    // 合法轮廓的最少顶点数
UNDER_SURFACE               = 0,    // 底平面
COVER_SURFACE               = 1     // 顶平面
} POLY_PARAMETEW;
...
{ POLY_TYPE_MAST            = 1,    // 柱体(普通物件轮廓)
POLY_TYPE_TRAPEZIA          = 2,    // 棱体(阶梯类有高度物件轮廓)
POLY_TYPE_HORIZONTAL        = 3,    // 水平面(地毯类物件轮廓)
POLY_TYPE_VERTICAL          = 4,    // 垂面(张贴类物件轮廓)
POLY_TYPE_BEVEL             = 5,    // 斜面(桥面类物件轮廓)
} POLY_TYPE_VALUE;
...
typedef struct tagPOLY
{
   ...
   SLONG    by;             // 重心点坐标 Y
   SLONG    bz;             // 重心点坐标 Z(Z1)
   SLONG    bu;             // 重心点坐标 Z0(under)

   SLONG    px[MAX_POLY_NUM+1]; // 顶点坐标 X
   SLONG    py[MAX_POLY_NUM+1]; // 顶点坐标 Y
   SLONG    pz[MAX_POLY_NUM+1]; // 顶点坐标 Z(Z1)
   SLONG    pu[MAX_POLY_NUM+1]; // 顶点坐标 Z0(under Z)
   SLONG    pa[MAX_POLY_NUM+1]; // 顶点属性(attrib)

   SLONG    na[2];          // 底(顶)平面法向量(X分量)
   SLONG    nb[2];          // 底(顶)平面法向量(Y分量)
   SLONG    nc[2];          // 底(顶)平面法向量(Z分量)
   SLONG    nd[2];          // 底(顶)平面方程参数 D ( na*X + nb*Y + nc*Z + nd = 0)
