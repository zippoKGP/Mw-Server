...
// --- initialize 
	...
	if(initialize_title_data()!=TTN_OK)
	{
		display_error_message((UCHR *)"Initialize Title data Error !",true);
		return(RETURN_ERROR);
	}
...
SLONG initialize_game_control(void)
{
	...
	game_control.refresh_screen_timer=0;
...
SLONG read_system_image(void)
{
	...
	MOUSE_DATA_TABLE mouse_data[10]={
...
SLONG initialize_chat_data_buffer(void)
{
	...
	for(i=0;i<MAX_CHAT_RECORD;i++)
	{
		...
		chat_data_buffer.record[i].channel=CHAT_CHANNEL_MESSAGE;
		...
		hirstory_chat_data_buffer.record[i].channel=CHAT_CHANNEL_MESSAGE;
...
// --- ( 0 - 1 ) initialize small map data
	...
	small_map_data.x_percent=1;
	small_map_data.y_percent=1;
	small_map_data.display_x_percent=1;
	small_map_data.display_y_percent=1;
...
// --- ( 0 - 0 ) Clear character base data
	...
	game_control.character_magic_id=-1;		// 清楚主角默认技能
	game_control.baobao_magic_id=-1;		// 清除宝宝默认技能
...
SLONG initialize_title_data(void)
{
	SLONG i;

	base_title_data.active=false;
	base_title_data.start_index=0;
	base_title_data.select_index=-1;
	for(i=0;i<MAX_TITLE;i++)
	{
		base_title_data.data[i].type=0;
		strcpy((char *)base_title_data.data[i].name,"");
		strcpy((char *)base_title_data.data[i].inst,"");
	}
