...
void get_version_string(char *text,int version_no0,int version_no1,int version_no2,char *dd,char *tt)
{
//	ULONG year=0,mon=0,da=0,hh=0,mm=0;
//	UCHR value[256];	
//	SLONG idx;
//	ULONG total;
/*
	...
	idx=0;
	get_part_str1((UCHR *)dd,value,&idx,' ',200);
	idx++;
	get_part_str1((UCHR *)dd,value,&idx,' ',200);
	idx++;
	da=atol((char *)value);
	get_part_str1((UCHR *)dd,value,&idx,' ',200);
	idx++;
	year=atol((char *)value);
	

	idx=0;
	get_part_str1((UCHR *)tt,value,&idx,':',200);
	idx++;
	hh=atol((char *)value);
	get_part_str1((UCHR *)tt,value,&idx,':',200);
	idx++;
	...
	total=(year-2005)*525600+mon*43200+da*1440+hh*60+mm;
*/
	
//	sprintf(text,"%d.%d.%d - %u",version_no0,version_no1,version_no2,total);
	sprintf(text,"%d.%d.%d - %d",version_no0,version_no1,version_no2,AUTO_ADD_ONE);
...
SLONG play_music_file(UCHR *filename,SLONG loop,SLONG volume)
{
	...
	if(game_control.music_playing==1)		// 同一首歌
	{
...
SLONG read_mda_frame(UCHR *mda_filename,SLONG frame_no,UCHR **buffer)
{
	...
	fseek(fp,temp_mda_head.data_start_offset,SEEK_SET);	//SEEK_SET=0 移动到数据开始的位置
	int cc = sizeof(struct INDEX_OFFSET_SIZE_STRUCT)*temp_mda_head.total_database_frame;
	index=(struct INDEX_OFFSET_SIZE_STRUCT *)malloc(cc);//sizeof(struct INDEX_OFFSET_SIZE_STRUCT)*temp_mda_head.total_database_frame);
	if(index==NULL)
	{
		display_error_message((UCHR *)MSG_MEMORY_ALLOC_ERROR,true);
		fclose(fp);
	...
	fread(index,cc,1,fp);//sizeof( struct INDEX_OFFSET_SIZE_STRUCT)*temp_mda_head.total_database_frame,1,fp);
...
SLONG check_map_layer_touch(SLONG real_x,SLONG real_y)
{
	...
	for(i=0;i<map_layer_touch.total;i++)
	{
		...
		if((real_x<map_base_data.map_layer_data[idx].orig_x)||
			(real_x>=map_base_data.map_layer_data[idx].orig_x+map_base_data.map_layer_data[idx].xlen)||				// 没有相交
...
static SERVERINFO server_list[] = { 
	{ "联竣东方", 1000, UGCOLOR_ARGB(211,161,197,119), 9009, 0, 0},
	{ "联竣西方", 1001, UGCOLOR_ARGB(211,161,197,120), 9009, 80, 1},
	{ "联竣独孤", 1002, UGCOLOR_ARGB(211,161,197,121), 9009, 20, 1},
	{ "联竣天上", 1003, UGCOLOR_ARGB(211,161,197,122), 9009, 40, 1},
	{ "联竣冬至", 2000, UGCOLOR_ARGB(211,161,197,123), 9009, 60, 1},
	{ "联竣夏至", 2001, UGCOLOR_ARGB(211,161,197,124), 9009, 100, 1},
	{ "联竣秋香", 3000, UGCOLOR_ARGB(211,161,197,125), 9009, 0, 1},
	{ "联竣春香", 3001, UGCOLOR_ARGB(211,161,197,126), 9009, 0, 1},
	{ "联竣白菜", 4000, UGCOLOR_ARGB(211,161,197,127), 9009, 0, 1}
};
*/
...
SLONG load_map_file(UCHR *filename,SLONG center_x,SLONG center_y)
{
	...
	for(i=0;i<temp_map_img_head.total_img;i++)
	{
		...
		map_base_data.map_layer_data[i].size=temp_img_head.size;	//图片尺寸
		map_base_data.map_layer_data[i].xlen=temp_img_head.xlen;	//长
		map_base_data.map_layer_data[i].ylen=temp_img_head.ylen;	//宽
		map_base_data.map_layer_data[i].orig_x=temp_img_head.orig_x;	// 原始座標位置
		map_base_data.map_layer_data[i].orig_y=temp_img_head.orig_y;
		map_base_data.map_layer_data[i].center_x=temp_img_head.center_x;	// 重心點座標
		map_base_data.map_layer_data[i].center_y=temp_img_head.center_y;
		map_base_data.map_layer_data[i].type=temp_img_head.type&0xC0;	//图片类型如：底图，中间层，底层，上层及该层第几张图
		// ---- layer image		
	...
	if(map_base_data.total_map_effect>0)
	{
		...
		for(i=0;i<map_base_data.total_map_effect;i++)
		{
			...
			if(map_base_data.map_effect_data[i].times==MAP_EFFECT_IMG)
			{
				...
				if(mda_group[img_idx].img256_head!=NULL)
				{
...
// --- ( 6 - 0 ) Create Find Path Bar	 整张地图寻径
...
// --- ( 0 - 2 ) 调整坐标
	oper_x=start_x/SCREEN_WIDTH*SCREEN_WIDTH;		// 左上角
...
void create_small_map(void)
{
	...
	game_control.window_world_map=-1;					// 世界地图
	...
	small_map_data.display_x_percent=(float)map_base_data.map_width/(float)SMALL_MAP_WIDTH;
	small_map_data.display_y_percent=(float)map_base_data.map_height/(float)SMALL_MAP_HEIGHT;

	small_map_data.point_x=-1;
	small_map_data.point_y=-1;
...
// --- ( 1 - 0 ) Cehck 	In Border 
	...
	{
		if(map_npc_group[system_control.control_npc_idx].npc_info.speed==0)	// 走路
			x_move_offset=4;
		else
...
void move_map_background(SLONG move_dir,SLONG step)
{
	...
	if(map_base_data.map_layer_data[0].image==NULL)		// 空的 IMAGE
		return;
...
SLONG check_need_load_background_buffer(SLONG x,SLONG y)
{
	...
	for(i=0;i<MAP_MAX_BOTTOM_BUFFER;i++)
	{
		if((x>=map_base_data.map_bottom_image[i].image_rect.left)&&
			...
			return(TTN_NOT_OK);		// 不用Load

	}

	return(TTN_OK);					// 需要 load 
}
...
void redraw_map_background(BMP *buffer,SLONG sx,SLONG sy)
{
	...
	for(i=0;i<MAP_MAX_BOTTOM_BUFFER;i++)
	{
		if(check_box_intersect(screen_rect.left,screen_rect.top,screen_rect.right,screen_rect.bottom,
			...
			map_base_data.map_bottom_image[i].image_rect.top,
			map_base_data.map_bottom_image[i].image_rect.right,
			map_base_data.map_bottom_image[i].image_rect.bottom)==TTN_OK)	// 相交
		{
...
void process_map_task(void)
{
	...
	for(i=0;i<total_map_npc;i++)
	{
		...
		{
		case NPC_MOVE_PATH:					// 循径移动
			ret_val=npc_auto_path_move(i);
			break;
		case NPC_MOVE_AUTO:					// 在格子(编辑器设定)里面移动
		case NPC_MOVE_HOLD:					// 不移动
...
// -------------- ret_val  如果是 TTN_OK 进入 Idle control 
		if((ret_val==TTN_OK)||
			...
			(ret_val==FIND_FAIL_ERROR))
		{
			...
			case NPC_MOTION_STANDBY2:
				...
				{
					rand_times=(rand()%10)*1000+NPC_IDLE_BASE;			// 5 秒
...
// --------- Exec Map Wav Effect 
	for(i=0;i<map_base_data.total_map_effect;i++)
	{
		...
		if(map_base_data.map_effect_data[i].times==MAP_EFFECT_WAV)
		{
			...
			{					// 进入 听得到的距离
				if(map_base_data.map_effect_data[i].wav_active==0)	// 第一次进入
				{
			...
			{
				if(map_base_data.map_effect_data[i].wav_active==1)	//  以前播放过
				{
					if(map_base_data.map_effect_data[i].channel>=0)	
...
// --------- 检查 没有主人的宝宝  一次检查一个#83
	for(i=0;i<total_map_npc;i++)
	{
		if(map_npc_group[i].npc_info.move_type==NPC_MOVE_FOLLOW)
		{
			...
			if((idx<0)||(idx>=total_map_npc))			// 主人脱离当前
			{
...
// --- 第一次执行
	...
	map_npc_group[map_npc_index].follow_id=-1;										// 没有跟随目标
...
// --- ( 0 - 1 ) get map_npc_group_index
	...
	if((map_npc_group_index<0)||(map_npc_group_index>=total_map_npc))return;
...
// --- Get Base Data 
	...
	{
		check_x=map_base_data.map_effect_data[idx].x;
		check_y=map_base_data.map_effect_data[idx].y;
	}
...
// --- Get Base Data 
	...
	if(mda_group[npc_mda_index].img256_head==NULL)return;
...
// --- Get Base Data 
	...
	if(ret_val==MDA_EXEC_LAST)
	{
		...
		if(extra_mda_data[idx].auto_close)			// 自动关闭
		{
			if(!extra_mda_data[idx].loop)			// 如果不是 loop
			{
				mda_group_close(&extra_mda_data[idx].handle);		// 关闭
...
SLONG npc_auto_follow_move(SLONG idx)
{
	...
	if((map_npc_idx<0)||(map_npc_idx>=total_map_npc))	// 主人脱离当前
	{
		return(TTN_NOT_OK);
	}
...
// -------- Check is out of range 
	...
	sx=map_npc_group[idx].npc_info.x;				// 宝宝位置
	sy=map_npc_group[idx].npc_info.y;
	tx=map_npc_group[map_npc_idx].npc_info.x;		// 跟随目标位置
	...
	if(offset<FOLLOW_JUMP_RANGE)					// 在跟随距离里面
	{
		...
		get_near_position(sx,sy,range,dir,&tx,&ty);	// 取得跟随目标
	...
	else											// 离开跟随距离
	{
		...
		map_npc_group[idx].npc_info.tx=tx;			// 直接跳耀
		map_npc_group[idx].npc_info.ty=ty;
...
SLONG npc_auto_path_move(SLONG map_npc_group_index)
{
	...
	// --- ( 0 - 1 ) Find
	if(map_npc_group[map_npc_group_index].npc_info.speed==0)	// 走路
...
//			postNetMessage(NET_MSG_PLAYER_MOVE_END, 0);			
		...
		break;
	case FIND_OK_CONTINUE:		//  找到目的地,但還未到達
	case FIND_OK_TOWARDS:		//  未找到目的地,只向目的地方向移動
...
//			move_dir=map_npc_group[map_npc_group_index].npc_info.dir;
		...
		break;
	case FIND_FAIL_NO_PATH:		//  沒有阻擋層,起始點或終點不在阻擋層內,起始點是阻擋格
	case FIND_FAIL_ERROR:		//  失敗
		change_npc_motion(map_npc_group_index,NPC_MOTION_STANDBY1);
		map_npc_group[map_npc_group_index].npc_idle_timer=system_control.system_timer+NPC_IDLE_BASE;
		map_npc_group[map_npc_group_index].npc_info.tx=map_npc_group[map_npc_group_index].npc_info.x;
		map_npc_group[map_npc_group_index].npc_info.ty=map_npc_group[map_npc_group_index].npc_info.y;
...
void change_npc_motion(SLONG idx,SLONG motion)
{
	...
	if((map_npc_group[idx].npc_type==NPC_CPU_TYPE)&&(motion==NPC_MOTION_RUN))	// 强迫 CPU_NPC 走路
		motion=NPC_MOTION_WALK;

	if((map_npc_group[idx].npc_type==NPC_BAOBAO_TYPE)&&(motion==NPC_MOTION_RUN))	// 强迫 CPU_NPC 走路
		motion=NPC_MOTION_WALK;
...
SLONG npc_auto_cell_move(SLONG idx)
{
	...
	case NPC_STAGE_NONE:
		if(system_control.system_timer<map_npc_group[idx].npc_idle_timer)
			return(TTN_NOT_OK);
		rand_times=(rand()%5)*1000+1000;			// 5 秒
		...
		case 0:										// 换成走路
			...
			rand_times=(rand()%10)*1000+1000;			// 5 秒
			map_npc_group[idx].npc_idle_timer=system_control.system_timer+rand_times;
			return(TTN_NOT_OK);
		case 1:
...
void update_player_move(PLAYER_MOVE& pp)
{
	...
	if((idx<0)||(idx>=total_map_npc))return;			// not find
...
SLONG add_extra_mda(UCHR *filename,SLONG x,SLONG y,bool loop,bool auto_close,bool hight_light,SLONG level,UCHR *command)
{
	...
	for(i=0;i<MAX_EXTRA_MDA;i++)
	{
		if(extra_mda_data[i].handle<0)		// 没人用
		{
			index=i;
			break;
		}
	...
	if(mda_group[extra_mda_data[index].mda_index].img256_head!=NULL)
	{
	...
	{
		extra_mda_data[index].xl=1;
		extra_mda_data[index].yl=1;
		extra_mda_data[index].center_x=1;
		extra_mda_data[index].center_y=1;
	}
...
void auto_close_extra_mda(void)
{
	...
	for(i=0;i<MAX_EXTRA_MDA;i++)
	{
		if(extra_mda_data[i].handle>=0)		// 没人用
		{
			if(extra_mda_data[i].auto_close==true)		// 需要自动关闭
			{
...
SLONG read_mda_button(SLONG handle,SLONG command_no,BUTTON_STATUS *button,UCHR *inst,SLONG code)
{
	...
	if(mda_group_exec(handle,command_text,-9999,-9999,screen_channel1,MDA_COMMAND_FIRST,0,0,0)==MDA_EXEC_ERROR)
		return(TTN_NOT_OK);
	if(mda_group[mda_index].img256_head==NULL)return(TTN_NOT_OK);
...
SLONG i;
	...
	if(game_control.chat_stack_index>=MAX_CHAT_STACK)	
		game_control.chat_stack_index=0;				// loop 回来
}
...
void push_chat_data(SLONG channel,ULONG id,UCHR *nickname,UCHR *text)
{
	...
	if(nickname!=NULL)
		...
		strcpy((char *)chat_data_buffer.record[idx].nickname,"");
...
void update_friend_list(CHAT_FRIEND_LIST data)
{
	...
	case FRIEND_NORMAL:									// 一般好友
		...
		break;
	case FRIEND_TEMP:									// 临时好友
		sprintf((char *)print_rec,MSG_ADD_TEMP_FRIEND,data.nickname);
		break;
	case FRIEND_MASK:									// 黑名单
		sprintf((char *)print_rec,MSG_ADD_DARK_FRIEND,data.nickname);
		break;
	default:
		sprintf((char *)print_rec,MSG_ADD_FRIEND,data.nickname);
		break;
...
void update_friend_list_online(ULONG friend_id,char online)
{
	...
	for(i=0;i<MAX_CHAT_FRIEND_LIST;i++)
	{
		...
		if(chat_friend_list[i].id==friend_id)
		{
			...
			{
			case 0:
				sprintf((char *)print_rec,MSG_FRIEND_OFFLINE,get_nexus_string(chat_friend_list[i].nexus,chat_friend_list[i].amity),(char *)chat_friend_list[i].nickname);
				push_chat_data(CHAT_CHANNEL_MESSAGE,0,(UCHR *)"",print_rec);
				break;
			case 1:
				sprintf((char *)print_rec,MSG_FRIEND_ONLINE,get_nexus_string(chat_friend_list[i].nexus,chat_friend_list[i].amity),(char *)chat_friend_list[i].nickname);
				push_chat_data(CHAT_CHANNEL_MESSAGE,0,(UCHR *)"",print_rec);
				break;
			}
		reload_chat_friend_list();
...
void delete_friend_list(ULONG friend_id)
{
	...
	for(i=0;i<MAX_CHAT_FRIEND_LIST;i++)		// clear temp
	{
		...
		display_friend_list[i].friend_type=0;					// 一般好友
		display_friend_list[i].nickname[0]=NULL;				// nicknane
		display_friend_list[i].title[0]=NULL;					// 称谓
		display_friend_list[i].level=-1;						// 等级
		display_friend_list[i].base_type=0;					// 基本形态
		display_friend_list[i].group[0]=0;						// 公会
		display_friend_list[i].nexus=0;						// 关系
		display_friend_list[i].amity=0;						// 友好度
...
void touch_system_npc(SLONG player_id,SLONG npc_id,SLONG touch_type)
{
// --- add touch type, see DATA.H
	postNetMessage(NET_MSG_PLAYER_MACRO_NPC, npc_id, (char*)&touch_type, sizeof(touch_type));
...
// -------- 送出 show baobao id, 回来时候 update show flag, 并呼叫 add_baobao_show(new_baobao);
...
void add_baobao_show(CLIENT_BAOBAO_SHOW baobao_data)
{
	...
	map_npc_idx=get_map_npc_index(follow_id);
	if((map_npc_idx<0)||(map_npc_idx>=total_map_npc))			// 主人已经不在当前银幕
		return;

	npc_data.id=baobao_data.id;							// ID
	strcpy((char *)npc_data.name,baobao_data.name);		// 昵称

	sprintf((char *)title,MSG_BAOBAO_TITLE,map_npc_group[map_npc_idx].npc_info.name);		// 称谓
	if(strlen((char *)title)>MAX_TITLE_LENGTH)
		npc_data.title[0]=NULL;
	else
		strcpy((char *)npc_data.title,(char *)title);
	npc_data.filename_id=baobao_data.filename_id;		// MDA 档案名称 id
	npc_data.base_type=0;								// 基本形态 0 到 8 
	npc_data.speed=0;									// 跑步或者走路 0 -> 走路 1 -> 跑步
	npc_data.motility=0;								// 移动类型
	npc_data.dir=NPC_DIR_DOWN;							// 方向
	npc_data.move_type=NPC_MOVE_FOLLOW;					// 移动方式
	npc_data.status=NULL;								// NPC 目前状态(显示)
	npc_data.life_base=baobao_data.life_base;			// 目前几转


	follow_count=get_map_npc_follow_conut(follow_id);	// 计算目前有多少人跟随这个id
	sx=map_npc_group[map_npc_idx].npc_info.x;
	sy=map_npc_group[map_npc_idx].npc_info.y;
	dir=map_npc_group[map_npc_idx].npc_info.dir;
	...
	if((map_npc_idx>=0)||(map_npc_idx<total_map_npc))
	{
...
{
	SLONG i;
	bool temp_show;											// 宝宝是否显示
	SERVER_CHARACTER_BAOBAO temp_baobao_list;				// 所带的宝宝当前资料
...
//	送 character_point_back 资料给 server,回来时候请 update base_character_data, 并且清除 character_point_back 资料
...
{
	// --- Send adjust point data
	//	送 baobao_point_back 资料给 server,回来时候请 update baobao_character_data[], 并且清除 baobao_point_back[] 资料
...
{
// --- 回来时候请 update character_fastness_data.active=true;
	postNetMessage(NET_MSG_PLAYER_FASTNESS, 0);
}

void request_baobao_fastness_data(void)
{
// --- 回来时候请 update baobao_fastness_data.active=true;
...
void request_character_pocket_data(void)
{
	...
	// --- 回来时候请 update character_pocket_data.active = true 
...
void send_character_item_use(SLONG item_idx)
{
	...
	// ------ Send item use (idx)
	// --- 回来时候请 呼叫 delete_character_item_pocket
...
{																													}
********************************************************************************************************************/
void request_deal(void)			// 要求与对方交易
{
	ULONG id;
	// --- Send request deal 要求与对方交易
	// 成功 呼叫 open_window_deal, 如果呼叫 open_window_deal==TTN_NOT_OK ,目前正在交易
	// 不成功 display_system_message("无法与对方交易,可能对方正在交易,战斗,或者组队当中!");
...
void send_cancel_deal(void)
{
	...
	// --- Send cancel deal 要求取消交易
	// 不管成功与否 呼叫 close_window_deal
	id=game_control.game_deal_id;
	if(id==0)return;
...
SLONG open_window_deal(void)
{
	...
	if(game_control.window_deal>=0)			// 正在交易
		return(TTN_NOT_OK);
...
*/	
	...
	if (game_control.game_give_id != 0)
	{
		...
		{
		case NPC_CPU_TYPE:
			system_control.macro_npc_id=game_control.game_give_id;
			break;
		case NPC_BAOBAO_TYPE:
		case NPC_CHARACTER_TYPE:
			break;
...
void refresh_all_map_npc(void)							// 重新要求地图所有 NPC
{
...
//	store_data.idx;			// 购买 idx
//	store_data.buy_number;	// 数量
...
{
// ---- Send 要求当铺资料  
// 回来时候请 update  character_popshop_data.active=true;
// 资料读入 character_popshop_data 
...
//	detail_item_inst.type;		种类
//	detail_item_inst.idx;		idx
//	sprintf((char *)print_rec,"名称%d->%d,%d#E说明资料#E装备条件#Ecakscjascjasc#Eakdcvjsdlvk#Ecsdvsdvjksd#Ecsdvsdv#Evsdvsdv#E#E#Ecdscvsd",detail_item_inst.type,detail_item_inst.idx,detail_item_inst.old_idx);
//	display_system_message(print_rec);
...
{
// ---- 传送 request skill data
// --- 回来时候请 update  game_control.character_skill_data_ready=true;
...
{
// ---- 传送 request baobao skill data
// --- 回来时候请 update  game_control.baobao_skill_data_ready=true;
...
//	delete_character_item_pocket(item_idx,1);
	...
	postNetMessage(NET_MSG_PLAYER_PET_USE_ITEM, baobao_id, (char*)&item_idx, sizeof(item_idx));
}
...
void fight_charge_start(void)				// 战斗开始充电	,进入战斗第一次呼叫
{
	charge_time_control(FIGHT_COMMAND_CHARACTER,CHARGE_TIME_INITIALIZE);	// 清除
	charge_time_control(FIGHT_COMMAND_BAOBAO,CHARGE_TIME_INITIALIZE);		// 清除
	charge_time_control(FIGHT_COMMAND_CHARACTER,CHARGE_TIME_CONTINUE);		// 开始
	charge_time_control(FIGHT_COMMAND_BAOBAO,CHARGE_TIME_CONTINUE);			// 开始
}

void fight_charge_pause(void)				// 战斗暂停充电
{
	charge_time_control(FIGHT_COMMAND_CHARACTER,CHARGE_TIME_PAUSE);	// 暂停
}

void fight_charge_continue(void)			// 战斗继续充电
{
	charge_time_control(FIGHT_COMMAND_CHARACTER,CHARGE_TIME_CONTINUE);	// 暂停
}
...
//sprintf((char *)print_rec,"主角 command send (%d->%u)",character_fight_command_data.command,character_fight_command_data.target_id);
//display_system_message(print_rec);
	...
	memcpy(&backup_character_fight_command_data,&character_fight_command_data,sizeof(struct FIGHT_COMMAND_DATA_STRUCT));	// 备份资料
...
//sprintf((char *)print_rec,"宝宝 command send (%d->%u)",baobao_fight_command_data.command,baobao_fight_command_data.target_id);
//display_system_message(print_rec);
	...
	memcpy(&backup_baobao_fight_command_data,&baobao_fight_command_data,sizeof(struct FIGHT_COMMAND_DATA_STRUCT));	// 备份资料
...
void insert_title_data(PUBLIC_TITLE_BASE data)
{
	SLONG i;
	SLONG idx;
	...
	{
		if(base_title_data.data[i].type>=1)continue;
		idx=i;
		break;
	}
	...
	memcpy(&base_title_data.data[idx],&data,sizeof(struct PUBLIC_TITLE_BASE_STRUCT));

}


void delete_title_data(SHINT type)
{
	SLONG i;
	
	for(i=0;i<MAX_TITLE;i++)
	{
		if(base_title_data.data[i].type==type)
			base_title_data.data[i].type=0;
	}
	
}


SHINT get_title_index(SHINT type)
{
	SLONG i;
	SLONG idx;
...
void request_title_data(void)
{
// --- 传送要求 title 资料
// base_title_data.active=true;
	postNetMessage(NET_MSG_PLAYER_TITLE_LIST, 0);
}


void send_change_title(SHINT type)
{
// --- 传送更换要求 title 资料
// update  base_character_data.title, map_npc_group[].npc_info.title

	postNetMessage(NET_MSG_PLAYER_TITLE_CHANGE, type);
}


void send_hide_title(void )
{
// --- 传送更换要求 title 资料
// update  base_character_data.title, map_npc_group[].npc_info.title

	postNetMessage(NET_MSG_PLAYER_TITLE_HIDE, 0);
}



void send_attack_npc(ULONG id)
{
	SLONG idx;
// --- 传送攻击 NPC或者玩家 ID

//	postNetMessage(NET_MSG_PLAYER_ATTACK_NPC, id);
	...
	switch(map_npc_group[idx].npc_type)
	{
	case NPC_CPU_TYPE:		// 攻击触发 macro 
		touch_system_npc(system_control.control_user_id,id,TOUCH_ATTACK_TYPE);
		break;
	case NPC_CHARACTER_TYPE:	// 攻击玩家,进入PK 或者切磋

//---- Send change to fight 

		postNetMessage(NET_MSG_PLAYER_ATTACK_NPC, id);
		break;
	case NPC_BAOBAO_TYPE:		// 无法攻击宝宝
		break;
	}
...
void send_spectator_run(void)
{
// --- send spectator run
//	display_system_message((UCHR *)"test");

	postNetMessage(NET_MSG_PLAYER_LEAVE_ATTACK_VEIW, 0);
}
