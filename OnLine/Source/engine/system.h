...
// --- config
	...
	SLONG npc_display_flag;			//是否显示其他玩家(f2)?
...
// --- main loop
	...
	SLONG main_loop_select_npc_idx;	// 主回路选择NPC idx		//我觉得应该是玩家的id
	SLONG main_loop_select_npc_task;// 主回路选择NPC 的task		//应该是当前选择的任务（交易等）
...
// --- 战斗
	SLONG character_magic_id;		// 主角默认法术
	SLONG baobao_magic_id;			// 宝宝默认法术
	bool see_fight;					// 观战 flag	
	
// --- screen text
	ULONG screen_text_timer;		// SCREEN_TEXT 用的 更换表情 timer
	ULONG refresh_screen_timer;		// 荧幕刷屏 timer
...
typedef struct FIGHT_IMAGE_STRUCT
{
	...
	IMG256_GROUP fight_baobao_icon;			// 30   宝宝肖像
...
typedef struct SMALL_MAP_DATA_STRUCT
{
	...
	float x_percent;
	float y_percent;
	float display_x_percent;
	float display_y_percent;
