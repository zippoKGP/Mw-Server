...
HRESULT CChatEdit::OnEnter(void)
{
	...
	check_emote((UCHR *)&chat_str[0],(UCHR *)base_character_data.name);
	...
	{
		push_chat_data(CHAT_CHANNEL_MESSAGE,0,(UCHR *)"",(UCHR *)MSG_CHAT_TIMER_ERROR);
	}
...
bool CChatEdit::updateKey(void)
{
	...
	if (!bkey_press)
	{
		...
		if (key_system != 0)
		{
			if (!g_bImeActive || !g_bImeCandidate)  //modified by coolly
			{
...
HRESULT CNickNameBox::OnEnter(void)
{
	return S_OK;
}
