...
UG_LONG CGMManager::init(CDoorConfig* pDoorConfig)
{
	...
	if(mallocMemory())
	{
		printf("malloc memory error in CGMManager.\n");
		g_pLog->UGLog("malloc memory error in CGMManager.");
		return -1;
	}
	if(initTCPComm())
	{
		printf("initTCPComm error in CGMManager.\n");
...
UG_LONG CGMManager::render(UG_ULONG ulTimer)
{
	...
	for(it = m_mapGM.begin(); it != m_mapGM.end();)
	{
		...
		{
			if(m_ulRenderTimer - p->m_dwTimer > 30000)
			{
	...
	{
		CGM* p = (*it).second;
		m_pTCPComm->sendBuffer(p->m_pvConnect,0);
	}
	return 0;
}
...
UG_LONG CGMManager::initTCPComm()
{
	...
	if(!m_pTCPComm)
	{
		printf("create TCPComm interface failed.\n");
