...
UG_ULONG CTCPComm::recvData(UG_PVOID pvConnect,UG_LONG lRecvCount,double& dbRecv,double& dbRecvs)
{
	if(m_pServerComm)
	{
...
UG_ULONG CTCPComm::writeBuffer(PUGGCP_T pUGGcp,UG_PCHAR pchBuffer)
{
	if(m_pClientComm)
	{
		return ((CTCPClient*)m_pClientComm)->writeBuffer(pUGGcp,pchBuffer);
	}
	return -1;
}
...
UG_ULONG CTCPComm::sendBuffer()
{
	if(m_pClientComm)
	{
		return ((CTCPClient*)m_pClientComm)->sendBuffer();
	}
	return -1;
}

UG_ULONG CTCPComm::recvData(UG_LONG lRecvCount)
{
	if(m_pClientComm)
	{
...
UG_ULONG CTCPComm::sendProc(void)
{
	if (m_pServerComm)
	{
		return ((CTCPServer*)m_pServerComm)->sendProc();
	}
