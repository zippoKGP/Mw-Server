...
public:
	...
	virtual UG_ULONG		init(UG_PCHAR pchPathName,FnNetSvr pfnCB,UG_PVOID pvKey);
	virtual UG_ULONG		shutdown();
	virtual UG_ULONG		disconnectPlayer(UG_PVOID pvConnect);
	virtual UG_ULONG		writeBuffer(UG_PVOID pvConnect,UG_DWORD dwMsgID,UG_DWORD dwParam,UG_PCHAR pchData = NULL,UG_ULONG ulLen = 0);
	...
	virtual UG_ULONG		recvData(UG_PVOID pvConnect,UG_LONG lRecvCount,double& dbRecv,double& dbRecvs);
	virtual UG_ULONG		setPlayer(UG_PVOID pvConnect,UG_PVOID pvPlayer);
	virtual UG_ULONG		acceptPlayer();
	
	virtual UG_ULONG		sendProc(void);
