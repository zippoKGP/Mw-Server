...
#define POLYGON_COUNTER_CLASS_A		0x2d63282b
#define POLYGON_COUNTER_CLASS_B		0x5aad3048
...
public:
    ...
    Class_ID	  ClassID() {return Class_ID(POLYGON_COUNTER_CLASS_A, POLYGON_COUNTER_CLASS_B);}
...
GetAppData(Interface * ip, int id, TCHAR* def, TCHAR* val, int len)
{
    ...
    AppDataChunk *ad = node->GetAppDataChunk(Class_ID(POLYGON_COUNTER_CLASS_A,POLYGON_COUNTER_CLASS_B),
                                             UTILITY_CLASS_ID, id);
    if (!ad)
        _tcscpy(val, def);
    else
...
{
    SoundObj *node = ip->GetSoundObject();
    node->RemoveAppDataChunk(Class_ID(POLYGON_COUNTER_CLASS_A,POLYGON_COUNTER_CLASS_B),
                             UTILITY_CLASS_ID, id);
    TCHAR* buf = (TCHAR*) malloc(_tcslen(val)+1);
    _tcscpy(buf, val);
    node->AddAppDataChunk(Class_ID(POLYGON_COUNTER_CLASS_A,POLYGON_COUNTER_CLASS_B),
                          UTILITY_CLASS_ID, id,
                          _tcslen(val)+1, buf);
