                                       ...
                                       HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case IDC_VCUTIL_RADIOSITYSETUP:			
			IActionManager* actionMgr = theApplyVC.ip->GetActionManager();
			if (actionMgr) {
				ActionTable* table = FindActionTableFromName(actionMgr, 
					"ToneOperatorAndRadiosityActionTable");
				if (table) {
					ActionItem* item = FindActionItemFromName(table, "Advanced Lighting Panel");
					if (item)
						item->ExecuteAction();
				}
			}
        ...
        case IDC_VCUTIL_UPDATEALL:
            ...
            break;
        case IDC_VCUTIL_LIGHTING:
            theApplyVC.currentOptions.lightingModel = kLightingOnly;
            theApplyVC.UpdateEnableStates();
            break;
        case IDC_VCUTIL_SHADED:
            theApplyVC.currentOptions.lightingModel = kShadedLighting;
            theApplyVC.UpdateEnableStates();
            break;
        case IDC_VCUTIL_DIFFUSE:                  
            theApplyVC.currentOptions.lightingModel = kShadedOnly;
            theApplyVC.UpdateEnableStates();
            break;
        case IDC_MIX:
...
void ApplyVCUtil::UpdateEnableStates() {
    ...
    if (hPanel != NULL) {
		...
		EnableWindow(GetDlgItem(hPanel, IDC_VCUTIL_RADIOSITYONLY_REUSEILLUM), enableRadiosity &&
			currentOptions.lightingModel != kShadedOnly);
