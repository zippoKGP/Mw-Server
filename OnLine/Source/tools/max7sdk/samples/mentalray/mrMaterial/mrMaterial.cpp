...
imrShader* mrMaterial::GetMRShader(imrShaderCreation& shaderCreation) {
	...
	if(m_mtlPhen == NULL) {
		...
		imrShader* gbufferShader = shaderCreation.CreateShader(_T("max_base_GBuffer"), mtlName.data());
		...
		ReplaceReference(kRef_GBufferShader, ((gbufferShader != NULL) ? &gbufferShader->GetReferenceTarget() : NULL));
		ReplaceReference(kRef_PhotonShaderList, ((photonShaderListShader != NULL) ? &photonShaderListShader->GetReferenceTarget() : NULL));

		DbgAssert((m_mtlPhen != NULL) && (m_surfaceShaderListShader != NULL) && (m_gbufferShader != NULL) && (m_photonShaderListShader != NULL));
	}
...
void mrMaterial::ReleaseMRShader() {
	...
	ReplaceReference(kRef_GBufferShader, NULL);
...
void mrMaterial::TranslateParameters(imrShader* shader, TimeValue t, Interval& valid) {
	...
	if((m_mainPB != NULL) && (m_mtlPhen != NULL)) {
		...
		if(paramsPB != NULL) {
			...
			if(GetParamIDByName(paramID, _T("surface"), paramsPB)) {
				...
				// Create a shader list with surface->bump->gbuffer if possible
				if(m_surfaceShaderListShader != NULL) {
					...
					if(GetParamIDByName(shaderListParamID, _T("shaderList"), shaderListParamsPB)) {
						...
						if(m_gbufferShader != NULL) {
							DbgAssert(m_gbufferShader->GetReferenceTarget().SuperClassID() == TEXMAP_CLASS_ID);
							Texmap* gbufferTexmap = static_cast<Texmap*>(&m_gbufferShader->GetReferenceTarget());
							shaderListParamsPB->Append(shaderListParamID, 1, &gbufferTexmap);
						}
			...
			if(GetParamIDByName(paramID, _T("photon"), paramsPB)) {
				...
				if(m_photonShaderListShader != NULL) {
...
RefTargetHandle mrMaterial::GetReference(int i) {
	...
	case kRef_GBufferShader:
		return ((m_gbufferShader != NULL) ? &m_gbufferShader->GetReferenceTarget() : NULL);
...
void mrMaterial::SetReference(int i, RefTargetHandle rtarg) {
	...
	case kRef_GBufferShader:
		DbgAssert((rtarg == NULL) || IsIMRShader(rtarg));
		m_gbufferShader = GetIMRShader(rtarg);
		break;
	case kRef_AdvancedPB:
		DbgAssert((rtarg == NULL) || (rtarg->SuperClassID() == PARAMETER_BLOCK2_CLASS_ID));
