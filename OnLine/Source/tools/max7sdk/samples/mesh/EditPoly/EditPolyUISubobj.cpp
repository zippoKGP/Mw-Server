...
BOOL EditPolySubobjControlDlgProc::DlgProc (TimeValue t, IParamMap2 *pmap, HWND hWnd,
	...
	case WM_COMMAND:
		...
		case IDC_BRIDGE:
			...
			if ((currentMode != ep_op_bridge_border) && (currentMode != ep_op_bridge_polygon) &&
				...
				switch (mpMod->GetEPolySelLevel()) {
				case EPM_SL_BORDER: mpMod->EpModToggleCommandMode (ep_mode_bridge_border); break;
				case EPM_SL_FACE: mpMod->EpModToggleCommandMode (ep_mode_bridge_polygon); break;
				}
			}
			break;
		...
		case IDC_SETTINGS_BRIDGE:
			switch (mpMod->GetEPolySelLevel()) {
			case EPM_SL_BORDER: mpMod->EpModPopupDialog (ep_op_bridge_border); break;
			case EPM_SL_FACE: mpMod->EpModPopupDialog (ep_op_bridge_polygon); break;
			}
			break;
