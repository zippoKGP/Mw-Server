...
int EditPolyCreateFaceMouseProc::proc (HWND hwnd, int msg, int point, int flags, IPoint2 m) {
	...
	case MOUSE_POINT:
		...
		if (hr) {
			...
			for (int j=0; j<mpts.Count(); j++) if (vts[j] == nv) break;
...
int EditPolyPickBridgeTargetProc::proc (HWND hwnd, int msg, int point, int flags, IPoint2 m) {
	...
	case MOUSE_POINT:
		...
		theHold.Begin ();
		{
			...
			if (mpEPoly->GetEPolySelLevel() == EPM_SL_BORDER) mpEPoly->EpModBridgeBorders (targ1, targ2, hr->nodeRef);
			else mpEPoly->EpModBridgePolygons (targ1, targ2, hr->nodeRef);
		}
