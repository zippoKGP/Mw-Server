...
bool SmoothingGroupUIHandler::GetSmoothingGroups (EPolyMod *pMod, DWORD *anyFaces, DWORD *allFaces, bool useSel) {
	...
	*anyFaces = 0;
	...
	for (int i=0; i<list.Count(); i++)
	{
		...
		BitArray fsel = pData->GetFaceSel();

		for (int j=0; j<pMesh->numf; j++)
		{
			if (useSel) {
				if (useStackSelection) {
					if (!pMesh->f[j].GetFlag (MN_EDITPOLY_STACK_SELECT)) continue;
				} else {
					if (!fsel[j]) continue;
				}
			}
			if (pMesh->f[j].GetFlag (MN_DEAD)) continue;
...
BOOL SurfaceDlgProc::DlgProc (TimeValue t, IParamMap2 *map, HWND hWnd,
