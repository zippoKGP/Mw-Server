	...
	if (macroRecorder->Enabled())
	{
		...
		if (pNode != NULL)
		{
			macroRecorder->FunctionCall(_T("$.modifiers[#Edit_Poly].DivideFace"), 2, 1,
				mr_int, face+1, mr_point3, &bary, _T("node"), mr_reftarg, pNode);
		}
		else
		{
			macroRecorder->FunctionCall(_T("$.modifiers[#Edit_Poly].DivideFace"), 2, 0,
				mr_int, face+1, mr_point3, &bary);
		}
		macroRecorder->EmitScript();
	}
}
...
void EditPolyMod::EpModSetBridgeNode (INode *pNode) {
	EditPolyData *pData = GetEditPolyDataForNode (pNode);
	if (selLevel == EPM_SL_BORDER) pData->SetPolyOpData (ep_op_bridge_border);
	else pData->SetPolyOpData (ep_op_bridge_polygon);
...
INode *EditPolyMod::EpModGetBridgeNode () {
	...
	for (int i=0; i<list.Count(); i++) {
		EditPolyData *pData = (EditPolyData *) list[i]->localData;
		if (pData == NULL) continue;
		if (pData->GetPolyOpData() == NULL) continue;
		int opid = pData->GetPolyOpData()->OpID();
		if ((opid == ep_op_bridge_border) || (opid == ep_op_bridge_polygon)) {
...
bool EditPolyMod::EpModReadyToBridgeSelected () {
	if (selLevel<EPM_SL_BORDER) return false;
	if (selLevel>EPM_SL_FACE) return false;
	bool brdr = (selLevel == EPM_SL_BORDER);
...
void EditPolyMod::EpModDragAccept (INode *pNode, TimeValue t) {
	...
	int nodeStart = (j<mPointNode.Count()) ? mPointRange[j] : 0;
...
//#endif
	...
	if (AreWeKeying(t) && (j<mPointNode.Count())) {
	...
	for (int i=0; i<limit; i++) {
		...
		if (mPointControl.Count() && (j<mPointNode.Count())) {
			int k = i+nodeStart;
