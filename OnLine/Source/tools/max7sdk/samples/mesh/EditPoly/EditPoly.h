...
friend class EditPolyBackspaceUser;
	...
	// Static keyboard shortcut table:
	static EditPolyActionCB *mpEPolyActions;

	// Static list of available operations:
	static Tab<PolyOperation *> mOpList;

	// Current operation for this instance:
	PolyOperation *mpCurrentOperation;
	...
	static EditPolyShapePickMode *mpShapePicker;

	static bool mUpdateCachePosted;
	static DWORD mHitLevelOverride, mDispLevelOverride;
	static bool mForceIgnoreBackfacing, mSuspendConstraints, mHitTestResult, mIgnoreNewInResult;
	static bool mDisplayResult;

	static EditPolyBackspaceUser backspacer;

	// Pseudo-command-mode for slice plane:
	static bool mSliceMode;

	// Necessary in order to support sequential cuts:
	static int mLastCutEnd;

	// Attach, Detach, Create Shape options
	static int mAttachType, mCreateShapeType, mCloneTo;
	static bool mAttachCondense, mDetachToObject, mDetachAsClone;
	static TSTR mCreateShapeName, mDetachName;

	// UI for the operation-specific dialog proc:
	static IParamMap2 *mpDialogOperation, *mpDialogAnimate, *mpDialogSelect;
	static IParamMap2 *mpDialogSoftsel, *mpDialogGeom, *mpDialogSubobj, *mpDialogSurface;
	static IParamMap2 *mpDialogPaintDeform;
	static bool mRollupSelect, mRollupSoftsel, mRollupGeom;
	static bool mRollupSubobj, mRollupAnimate, mRollupSurface, mRollupPaintDeform;
...
public:
	...
	// Indicates if we can currently show a gizmo:
	bool ShowGizmoConditions ();
	// Indicates if we are currently showing a gizmo:
	bool ShowGizmo ();
	...
	//void EpModTurnEdge (int edge, INode *pNode=NULL);
	...
	void		ApplyPaintDeform( MeshPaintHost* host );
	void		RevertPaintDeform( MeshPaintHost* host );
	// notifications
...
friend class EditPolyMod;
	...
	BitArray mVertHide, mFaceHide;
	MNMesh *mpMesh, *mpMeshOutput;	// Simple copies of the mesh before and after modification
	...
	MNTempData *mpTemp, *mpTempOutput;
	...
	Interval mGeomValid, mTopoValid, mMeshValid;
...
public:
	...
	//MapBitArray GetActiveMaps () { return mActiveMaps; }
	...
	//float *GetSoftSelection () { return mSoftSelection.Count() ? mSoftSelection.Addr(0) : NULL; }
	float *GetSoftSelection (TimeValue t,
...
private:
	...
	bool mInBevel;
	float mStartOutline, mStartHeight;
...
private:
	...
	float mStartHeight, mStartWidth;
...
class EditPolyHingeFromEdgeProc : public MouseCallBack {
	EPolyMod *mpEPoly;
	IObjParam *ip;
	IPoint2 mFirstPoint, mCurrentPoint;
	HitRecord *mpHitRec;
	bool mEdgeFound;
	float mStartAngle;
...
enum { ep_animate, ep_select, ep_softsel, ep_geom,
		ep_subobj, ep_surface, ep_settings, ep_paintdeform };
...
{
private:
	Tab<int> mStartLevel, mStartIndex;
	Tab<Point3> mStart, mEnd, mNormal;
