...
void EditPolyObject::GetSubObjectTMs (SubObjAxisCallback *cb,TimeValue t,
	...
	case EP_SL_VERTEX:
		if (ip->GetCommandMode()->ID()==CID_SUBOBJMOVE) {
			...
			float *vsw = mm.getVSelectionWeights();
			for (int i=0; i<mm.numv; i++) if (mm.v[i].FlagMatch (MN_SEL|MN_DEAD, MN_SEL) || (vsw && vsw[i])) break;
			if (i >= mm.numv) return;
...
void EditPolyObject::EpPreviewCancel () {
	...
	if ((previewOp == epop_bridge_polygon) || (previewOp == epop_bridge_border)) ResetBridgeParams ();
