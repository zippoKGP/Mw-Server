...
static ParamBlockDesc2 ep_param_block ( ep_block, _T("ePolyParameters"),
	...
	ep_bridge_selected, _T("bridgeSelected"), TYPE_INT, P_TRANSIENT, IDS_BRIDGE_SELECTED,
		p_default, true,
		p_range, 0, 1,
...
BOOL SoftselDlgProc::DlgProc (TimeValue t, IParamMap2 *map, HWND hWnd,
	...
	case WM_COMMAND:
		...
		case IDC_PAINTSEL_OPTIONS:
			theMeshPaintMgr.BringUpOptions();
			break;
		}
		break;
...
void PreserveMapsUIHandler::Initialize (HWND hWnd) {
	...
	if (pMesh != NULL) {
		for (chan=-NUM_HIDDENMAPS; chan<pMesh->numm; chan++) {
	...
	case WM_COMMAND:
		...
		case IDC_BRIDGE:
			if (mpEditPoly->EpfnReadyToBridgeFlagged (EP_SL_CURRENT, MN_SEL)) {
				...
				switch (mpEPoly->GetEPolySelLevel()) {
				case EP_SL_BORDER: mpEPoly->EpActionToggleCommandMode (epmode_bridge_border); break;
				case EP_SL_FACE: mpEPoly->EpActionToggleCommandMode (epmode_bridge_polygon); break;
				}
			}
			break;
		...
		case IDC_SETTINGS_BRIDGE:
			switch (mpEPoly->GetEPolySelLevel()) {
			case EP_SL_BORDER: mpEditPoly->EpfnPopupDialog (epop_bridge_border); break;
			case EP_SL_FACE: mpEditPoly->EpfnPopupDialog (epop_bridge_polygon); break;
			}
			break;
...
BOOL PaintDeformDlgProc::DlgProc (TimeValue t, IParamMap2 *pmap, HWND hWnd,
						   UINT msg, WPARAM wParam, LPARAM lParam)
{
	//BOOL ret;
	EditPolyObject *pEditPoly = (EditPolyObject*)mpEPoly;
	...
	case WM_COMMAND:
		...
		case IDC_PAINTDEFORM_APPLY:
			if( pEditPoly->InPaintMode() ) pEditPoly->EndPaintMode();
			pEditPoly->DeactivatePaintData( PAINTMODE_DEFORM, TRUE );
			break;
		case IDC_PAINTDEFORM_CANCEL:
			if( pEditPoly->InPaintMode() ) pEditPoly->EndPaintMode();
			pEditPoly->DeactivatePaintData( PAINTMODE_DEFORM, FALSE );
			break;
...
void EditPolyObject::ApplyPaintDeform( MeshPaintHost* host ) {
	localGeomValid = NEVER;
	Point3* offsets = GetPaintDeformOffsets();
	int count = GetPaintDeformCount();
	MNMesh& mesh = GetMesh();
	// Add in the offset values
	for( int i=0; i<count; i++ ) mesh.v[i].p += offsets[i];
}

void EditPolyObject::RevertPaintDeform( MeshPaintHost* host ) {
	localGeomValid = NEVER;
	Point3* offsets = GetPaintDeformOffsets();
	int count = GetPaintDeformCount();
	MNMesh& mesh = GetMesh();
...
void EditPolyObject::OnParamSet(PB2Value& v, ParamID id, int tabIndex, TimeValue t) {
	...
	case ep_paintdeform_size:
		SetPaintBrushSize(v.f); break;
	case ep_paintsel_strength:
	case ep_paintdeform_strength:
		SetPaintBrushStrength(v.f); break;
	}
}
...
void EditPolyObject::OnPaintBtn( int mode, TimeValue t ) {
	...
	if( mode==GetPaintMode() ) EndPaintMode();
