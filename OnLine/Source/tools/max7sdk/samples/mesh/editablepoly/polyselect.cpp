...
public:
	...
	void		ApplyPaintDeform( MeshPaintHost* host ) {} //not supported
	void		RevertPaintDeform( MeshPaintHost* host ) {} //not supported
	// notifications
...
enum { ps_use_softsel, ps_use_edist, ps_edist, ps_affect_backfacing,
		...
		ps_matid, ps_lock_softsel, ps_paintsel_value, ps_paintsel_size, ps_paintsel_strength, };
...
void PolyMeshSelMod::OnParamSet(PB2Value& v, ParamID id, int tabIndex, TimeValue t) {
	...
	case ps_paintsel_size:		SetPaintBrushSize( v.f ); break;
	case ps_paintsel_strength:	SetPaintBrushStrength( v.f ); break;
	}
}
...
void PolyMeshSelMod::OnPaintBtn( int paintMode, TimeValue t ) {
	if( paintMode==GetPaintMode() ) EndPaintMode();
	else {
...
LocalModData *PolyMeshSelData::Clone() {
	...
	d->eselSet = eselSet;
	d->fselSet = fselSet;
	d->mPaintSelCount = mPaintSelCount;
