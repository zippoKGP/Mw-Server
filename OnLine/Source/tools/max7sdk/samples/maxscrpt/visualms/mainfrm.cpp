...
HWND CMainFrame::GetFrameHWnd(HWND hWnd)
{
	while(hWnd != NULL)
	{
		if((BOOL)::GetProp(hWnd, frameWndAtomID) == TRUE)
			return hWnd;
		hWnd = ::GetParent(hWnd);
	}
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	::SetClassLongPtr(
		hBar,
		GCLP_HBRBACKGROUND,
		(LONG)GetCustSysColorBrush(COLOR_BTNFACE));

	// Set the main window icon to match MAX's icon.
	SetIcon((HICON)GetClassLongPtr(GetCOREInterface()->GetMAXHWnd(), GCLP_HICONSM), FALSE);
...
void CMainFrame::OnFileExit() 
{
	...
	if(::IsWindow(hMainWnd))
	{
		...
		SetWindowLongPtr(hMainWnd, GWLP_HWNDPARENT, (LONG_PTR)NULL);
