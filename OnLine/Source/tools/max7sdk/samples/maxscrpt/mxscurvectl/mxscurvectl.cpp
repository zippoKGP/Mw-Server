...
{	
	...
	if (prop == n_name)
	{
		...
		curve->AddAppDataChunk(CURVE_CONTROL_CLASS_ID, REF_MAKER_CLASS_ID, 
				APPID_CURVENAME, strlen(data) + 1, data);		
	}
...
{
	MAXCurveCtl *mcc = (MAXCurveCtl*)GetWindowLongPtr(hWnd,GWLP_USERDATA);
...
//		}
	...
	SetWindowLongPtr(hFrame, GWLP_USERDATA, (LONG_PTR)this);
	SetWindowLongPtr(hFrame, GWLP_WNDPROC, (LONG_PTR)CCTL_WndProc);
