...
#define ATTACH_CONTROL_CLASSID		Class_ID(0xbb27e611,0xa72f43e7)
...
MAXAKey::MAXAKey(Control* icont, int ikey, ParamDimension* idim)
{
	...
	key_index = ikey;
//	isAttachment2=(controller->ClassID() == ATTACH2_CONTROL_CLASSID);
}
...
MAXAKey::setup_key_access()
{
	...
	return &((AttachCtrl*)controller->controller)->keys[key_index];
}
...
{
	deletion_check(controller);
	//	if (isAttachment2) ((AttachCtrl*)controller)->Invalidate();
	controller->controller->NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
	needs_redraw_set();
}
...
MAXAKey::get_props_vf(Value** arg_list, int count)
{
	...
	return new Array (0);  // hey!! implement me
}
...
{
	INode* theNode=((AttachCtrl*)obj)->node;
...
attach_set_node(ReferenceTarget* obj, Value* prop, TimeValue t, Value* val)
{
	...
	BOOL res=((AttachCtrl*)obj)->SetObject(theNode);
	if (!res)
			throw RuntimeError(GetString(IDS_ATTACH_SET_NODE_FAILED), val);
	obj->NotifyDependents(FOREVER,0,REFMSG_CHANGE);
	needs_redraw_set();
}
...
{
	return (((AttachCtrl*)obj)->align ? &true_value : &false_value);
}

void    
attach_set_align(ReferenceTarget* obj, Value* prop, TimeValue t, Value* val)
{
	((AttachCtrl*)obj)->align=val->to_bool();
	obj->NotifyDependents(FOREVER,0,REFMSG_CHANGE);
//	((AttachCtrl*)obj)->Invalidate();
	needs_redraw_set();
}
...
{
	return (((AttachCtrl*)obj)->manUpdate ? &true_value : &false_value);
}

void    
attach_set_manupdate(ReferenceTarget* obj, Value* prop, TimeValue t, Value* val)
{
	((AttachCtrl*)obj)->manUpdate=val->to_bool();
	obj->NotifyDependents(FOREVER,0,REFMSG_CHANGE);
//	((AttachCtrl*)obj)->Invalidate();
...
MAXClass attachment
	(_T("Attachment"), ATTACH_CONTROL_CLASSID, CTRL_POSITION_CLASS_ID, &position_controller, 0,
		accessors,
			fns,
...
Attachment_GetKey_cf(Value** arg_list, int count)
{
	...
	Control* c = arg_list[0]->to_controller();
	deletion_check(((MAXControl*)arg_list[0]));
	if (c->ClassID() != ATTACH_CONTROL_CLASSID)
...
Attachment_AddNewKey_cf(Value** arg_list, int count)
{
	...
	DWORD flags = 0;
	if (count == 3)
	{	if (arg_list[3] == n_select)
			flags |= ADDKEY_SELECT;
		else 
			throw TypeError (GetString(IDS_UNRECOGNIZED_FLAG_FOR_ADDNEWKEY), arg_list[3]);
	}
	c->AddNewKey(t, flags);
	needs_redraw_set();
...
Attachment_Update_cf(Value** arg_list, int count)
{
	...
	Control* c = arg_list[0]->to_controller();
	deletion_check(((MAXControl*)arg_list[0]));
	if (c->ClassID() != ATTACH_CONTROL_CLASS_ID)
		throw RuntimeError (GetString(IDS_NOT_ATTACH_CONTROL), arg_list[0]);
	c->NotifyDependents(FOREVER,0,REFMSG_CHANGE);
	((AttachCtrl*)c)->Invalidate();
	needs_redraw_set();
	return &ok;
}
*/
...
{
	return Integer::intern(((EulerRotation*)obj)->order+1);
}
...
euler_set_order(ReferenceTarget* obj, Value* prop, TimeValue t, Value* val)
{
	...
	((EulerRotation*)obj)->order=order-1;
    ((EulerRotation*)obj)->ivalid.SetEmpty();
    ((EulerRotation*)obj)->NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
    ((EulerRotation*)obj)->NotifyDependents(FOREVER, PART_ALL, REFMSG_SUBANIM_STRUCTURE_CHANGED);
	needs_redraw_set();
}
...
Value* 
local_euler_get_order(ReferenceTarget* obj, Value* prop, TimeValue t, Interval& valid)
{
	return Integer::intern(((EulerRotation*)obj)->order+1);
}

void    
local_euler_set_order(ReferenceTarget* obj, Value* prop, TimeValue t, Value* val)
{
	int order=val->to_int();
	range_check(order, 1, 9, GetString(IDS_AXIS_ORDER_INDEX_OUT_OF_RANGE));
	((EulerRotation*)obj)->order=order-1;
...
MAXClass local_euler_XYZ_rotation
	(_T("Local_Euler_XYZ"), Class_ID(LOCAL_EULER_CONTROL_CLASS_ID, 0), CTRL_ROTATION_CLASS_ID, &rotation_controller, 0,
		accessors,
			fns,
				_T("axisOrder"),	local_euler_get_order,	local_euler_set_order,	TYPE_UNSPECIFIED,
				end,
			end,
		end
	);
