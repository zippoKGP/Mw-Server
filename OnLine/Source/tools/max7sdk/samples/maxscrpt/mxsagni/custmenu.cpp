...
{
	MenuData *md = (MenuData*)GetWindowLongPtr(hWnd, GWLP_USERDATA);
	...
	for(int i = 0; i < count; i++)
	{
		...
		else
		{
			if(i == m_nSelected)
			{
				...
				TextOut(hDC, x, y+2, mi->m_sText, strlen(mi->m_sText));
				DeleteObject(hBrush);
			}
			else
			{
...
//	size.cx += c_textTrail;
	...
	for(int i = 0; i < count; i++)
	{
		...
		else
		{
			...
			GetTextExtentPoint32(GetDC(m_hWnd), mi->m_sText, strlen(mi->m_sText), &tmp);
