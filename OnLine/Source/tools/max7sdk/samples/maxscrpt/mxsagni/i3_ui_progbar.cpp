...
LRESULT CALLBACK ish3_ProgressBar::WndProc( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
{
	...
	ish3_ProgressBar *UGC = (ish3_ProgressBar*)GetWindowLongPtr(hWnd,GWLP_USERDATA);
	...
	switch ( message ) {
		...
		case WM_CREATE:
			{
				...
				SetWindowLongPtr(hWnd, GWLP_USERDATA, (LONG_PTR)UGC);
				break;
			}
		...
		if(UGC->type==TYPE_HORIZ)
		{
				...
				if(amt!=100)
				{
					SelectObject(hdc, hPT);
					SelectObject(hdc, hProgressTerm);
...
ish3_ProgressBar::get_property(Value** arg_list, int count)
{
	...
	if (prop == n_value)
		...
		return get_event_handler(n_clicked);
	else
