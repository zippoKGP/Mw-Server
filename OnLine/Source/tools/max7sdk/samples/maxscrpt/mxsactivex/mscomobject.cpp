...
// apply proper iCharOffset
	...
	// RK: 02/14/02 Hackola! for licensing MSCOMCTL.OCX
	bool already_licensed;
	TCHAR licKey[] = _T("Licenses\\ED4B87C4-9F76-11d1-8BF7-0000F8754DA1");
	TCHAR licVal[] = _T("knlggnmntgggrninthpgmnngrhqhnnjnslsh");
	HKEY hLic;

	already_licensed = (RegOpenKeyEx(HKEY_CLASSES_ROOT, licKey, 0, KEY_READ, &hLic) == ERROR_SUCCESS);

	if (!already_licensed)	
		RegSetValue(HKEY_CLASSES_ROOT, licKey, REG_SZ, licVal, _tcslen(licVal));

	hr = spAxWindow->CreateControl(pszName, hwndContainer, 0);
	def_hr_error(hr);

	if (!already_licensed)
		RegDeleteKey(HKEY_CLASSES_ROOT, licKey);	
...
ActiveXControl::get_property(Value** arg_list, int count)
{
	...
	{
		if(val = get_event_handler(arg_list[0]))
			return val;
	}
	return RolloutControl::get_property(arg_list, count);
}
...
MSComObject::show_events_vf(Value** arg_list, int count)
{
	...
	for (int i=0; i < num_props; i++)
	{
		...
		if (ik != INVOKE_FUNC) continue;
		buf.printf( _T("  on <control_name> %s"), pEventInfo->GetName() );
...
// global funcs
/////////////////////////////////////////////////////////////////////////////
def_visible_primitive(show_all_axcontrols, _T("ShowAllActiveXControls"));
Value*
...
def_visible_primitive(used_maps,					"usedMaps");
...
class ClearAFlagNodeEnum: public INodeEnum 
{
	int  proc(INode *node) 
	{
		ClearAFlagInHierarchy(node,A_WORK1);
		return 1;
	}
};
...
used_maps_cf(Value** arg_list, int count)
{
	...
	if (count > 0)
	{
		...
		ClearAFlagInHierarchy(obj,A_WORK1);
	...
	else
	{
		...
		// First clear all A_WORK1 flags
		ClearAFlagInHierarchy(scene,A_WORK1);
		ClearAFlagNodeEnum nodeClearEnum;
		INodeEnumTree(MAXScript_interface->GetRootNode(),nodeClearEnum);
...
load_picture_cf(Value** arg_list, int count)
{
	...
	LPDISPATCH picDisp;
	TCHAR* name = arg_list[0]->to_string();
	VARIANT var;
	V_VT(&var) = VT_BSTR;	
	
	USES_CONVERSION;
	V_BSTR(&var) = T2BSTR(name);
...
send_message_cf(Value** arg_list, int count)
{
	...
	HRESULT hRes = SendMessage(
		(HWND)arg_list[0]->to_int(),
		(UINT)arg_list[1]->to_int(),
		(WPARAM)arg_list[2]->to_int(),
		(LPARAM)arg_list[3]->to_int());
...
hittest_cf(Value** arg_list, int count)
{
	...
	HRESULT hRes = SendMessage((HWND)arg_list[0]->to_int(), LVM_SUBITEMHITTEST, 0, (LPARAM)&info);
...
get_indent_cf(Value** arg_list, int count)
{
	...
	HRESULT hRes = SendMessage((HWND)arg_list[0]->to_int(), LVM_GETITEM, 0, (LPARAM)&item);
...
set_indent_cf(Value** arg_list, int count)
{
	...
	HRESULT hRes = SendMessage((HWND)arg_list[0]->to_int(), LVM_SETITEM, 0, (LPARAM)&item);
	return Integer::intern(hRes);
}
