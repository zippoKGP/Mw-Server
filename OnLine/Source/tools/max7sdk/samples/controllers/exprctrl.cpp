...
void InitResizeData(HWND	hDlg,						
					float	&l_expr_scaling,			
					int		&l_expr_height_delta,		
					int		&l_expr_width_delta, 
					int		&l_descr_height_delta,
					float	&l_descr_scaling,
					int		&l_descr_width_delta, 
					int		&l_height_delta )
{		
	...
	// get the original position of the expression editor 
	GetWindowPlacement(GetDlgItem(hDlg, IDC_STATIC_EXPR), &l_resizeWindowPlacement);
	l_expr_scaling		= l_resizeWindowPlacement.rcNormalPosition.bottom;
	l_expr_height_delta	= l_resizeWindowPlacement.rcNormalPosition.top;
	l_expr_width_delta	= l_resizeWindowPlacement.rcNormalPosition.left;

	GetWindowPlacement(GetDlgItem(hDlg, IDC_EXPR_EDIT), &l_resizeWindowPlacement);
	l_expr_height_delta	= l_resizeWindowPlacement.rcNormalPosition.top - l_expr_height_delta;
	l_expr_width_delta	= l_resizeWindowPlacement.rcNormalPosition.left - l_expr_width_delta;

	// get the size of the lower static control 
	GetWindowPlacement(GetDlgItem(hDlg, IDC_STATIC_DESCR), &l_resizeWindowPlacement);
	l_descr_scaling			= l_resizeWindowPlacement.rcNormalPosition.top;
	l_descr_height_delta	= l_resizeWindowPlacement.rcNormalPosition.top;
	l_descr_width_delta		= l_resizeWindowPlacement.rcNormalPosition.left;

	l_height_delta			= l_resizeWindowPlacement.rcNormalPosition.bottom; 

	GetWindowPlacement(GetDlgItem(hDlg, IDC_DESCRIPTION), &l_resizeWindowPlacement);
	l_descr_height_delta	= l_resizeWindowPlacement.rcNormalPosition.top - l_descr_height_delta;
	l_descr_width_delta		= l_resizeWindowPlacement.rcNormalPosition.left - l_descr_width_delta;


	// get the original position of the expression editor 
	GetWindowPlacement(GetDlgItem(hDlg, IDC_FUNCTIONS), &l_resizeWindowPlacement);
	l_expr_scaling			/= l_resizeWindowPlacement.rcNormalPosition.bottom;
	l_descr_scaling			/= l_resizeWindowPlacement.rcNormalPosition.bottom;

	l_height_delta			= l_resizeWindowPlacement.rcNormalPosition.top - l_height_delta;
...
void ResizeDialog(HWND	hDlg, 
				  int	in_expr_height_delta, 
				  float in_descr_scaling, 
				  int	in_expr_width_delta,
				  float in_expr_scaling, 
				  int	in_descr_height_delta , 
				  int	in_descr_width_delta, 
				  int	in_height_delta)
{
	...
	// get the original position of the expression editor 
	GetWindowPlacement(GetDlgItem(hDlg, IDC_STATIC_EXPR), &l_resizeWindowPlacement);
	int l_P0_X		= l_resizeWindowPlacement.rcNormalPosition.left; 
	int l_P0_Y		= l_resizeWindowPlacement.rcNormalPosition.top; 

	// get the position of the function list button: the lower right position 
	// used for resizing 
	GetWindowPlacement(GetDlgItem(hDlg, IDC_FUNCTIONS), &l_resizeWindowPlacement);
	int l_P2_Y = l_resizeWindowPlacement.rcNormalPosition.top;
	int l_P2_X = l_resizeWindowPlacement.rcNormalPosition.right;

	// set the new control position: top left stay fixed , 
	// bottom is set to half of the size between the top of the function button 
	// and the top of the expression control . 
	l_resizeWindowPlacement.rcNormalPosition.top	= l_P0_Y;
	l_resizeWindowPlacement.rcNormalPosition.left	= l_P0_X;
	l_resizeWindowPlacement.rcNormalPosition.bottom = in_expr_scaling*l_P2_Y + l_P0_Y;
	l_resizeWindowPlacement.rcNormalPosition.right	= l_P2_X;
	SetWindowPlacement(GetDlgItem(hDlg, IDC_STATIC_EXPR), &l_resizeWindowPlacement);

	// update now the expression edit control : 
	l_resizeWindowPlacement.rcNormalPosition.top		+= in_expr_height_delta;
	l_resizeWindowPlacement.rcNormalPosition.left		+= in_expr_width_delta;
	l_resizeWindowPlacement.rcNormalPosition.bottom		-= in_descr_height_delta;
	l_resizeWindowPlacement.rcNormalPosition.right		-= in_descr_width_delta;
	SetWindowPlacement(GetDlgItem(hDlg, IDC_EXPR_EDIT), &l_resizeWindowPlacement);

	// resize the explain control 
	l_resizeWindowPlacement.rcNormalPosition.top	= l_resizeWindowPlacement.rcNormalPosition.bottom - 8;
	SetWindowPlacement(GetDlgItem(hDlg,IDC_EXPLAIN_NOTATION ), &l_resizeWindowPlacement);

	// resize the lower control: first the outer static 
	l_resizeWindowPlacement.rcNormalPosition.top	= in_descr_scaling*l_P2_Y + l_P0_Y;
	l_resizeWindowPlacement.rcNormalPosition.left	= l_P0_X;
	l_resizeWindowPlacement.rcNormalPosition.bottom = l_P2_Y  - in_height_delta;
	l_resizeWindowPlacement.rcNormalPosition.right	= l_P2_X ;
	SetWindowPlacement(GetDlgItem(hDlg, IDC_STATIC_DESCR), &l_resizeWindowPlacement);

	// resize the inner description control 
	l_resizeWindowPlacement.rcNormalPosition.top	+= in_descr_height_delta;
	l_resizeWindowPlacement.rcNormalPosition.left	+= in_descr_width_delta;
	l_resizeWindowPlacement.rcNormalPosition.bottom -= in_descr_height_delta;
	l_resizeWindowPlacement.rcNormalPosition.right	-= in_descr_width_delta;
...
static INT_PTR CALLBACK ExprParamsWndProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	static int				l_descr_width_delta		= 0;
	static int				l_descr_height_delta	= 0;
	static int				l_expr_width_delta		= 0;
	static int				l_expr_height_delta		= 0;
	static int				l_height_delta			= 0;
	static float			l_descr_scaling			= 1.0;
	static float			l_expr_scaling			= 1.0;
	switch (msg) {
	case WM_INITDIALOG:	
		SetWindowLongPtr(hDlg, GWLP_USERDATA, lParam);
		...
		resizer.SetControlInfo(IDC_CUR_ASGN ,		DialogResizer::kLockToBottomRight);
		...
		InitResizeData( hDlg, l_expr_scaling, l_expr_height_delta, l_expr_width_delta,
			l_descr_height_delta,l_descr_scaling,l_descr_width_delta, l_height_delta );
		...
		ResizeDialog(hDlg, l_expr_height_delta, l_descr_scaling, l_expr_width_delta,
					l_expr_scaling, l_descr_height_delta , l_descr_width_delta, l_height_delta);
