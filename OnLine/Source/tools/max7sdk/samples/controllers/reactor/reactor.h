...
class Reactor : public IReactor, public ResourceMakerCallback {
	...
	public:
		...
		MasterState* getMasterState(int i) {return GetMaster()->GetState(slaveStates[i].masterID);}
		...
		BOOL IsRealDependency(ReferenceTarget* rtarg) { if (rtarg == master) return FALSE; return TRUE; }
