...
ReactionMaster* ReactionManager::AddReaction(ReferenceTarget* owner, int subAnimIndex, Reactor* slave)
{
	if (!IsSuspended())
	{
		...
		MakeRefByID(FOREVER, i, (ReferenceTarget*)rs);
		mSelected = i;
...
void ReactionManager::MaybeAddReactor(Reactor* slave)
{
	...
	{
		ReactionMaster* newMaster = GetReactionManager()->AddReaction(slavesMaster->client, slavesMaster->subnum, slave);
		if (slavesMaster != newMaster)
		{
			// copy the states to the new master
			if (newMaster != NULL)
				newMaster->CopyFrom(slave);

			slave->ReplaceReference(0, newMaster);
			// The client may have been deleted when the old master was deleted.
			// If this happened the newMaster isn't auto deleted since it was locked
			// during ReplaceReference.  If its client was deleted delete the new master as well.
			if (newMaster && !newMaster->client)
			{
				newMaster->MaybeAutoDelete();
			}
		}
	}
	else
...
void ReactionSet::AddSlave(Reactor* r)
{
	...
	MakeRefByID(FOREVER, SlaveNumToRefNum(i), r);
	NotifyDependents(FOREVER, PART_ALL, REFMSG_SUBANIM_STRUCTURE_CHANGED);
}
