...
void Reactor::SetReference(int i, RefTargetHandle rtarg) 
{
	switch (i)
	{
		...
		case 2: // remapped on load.  (This is only applicable to loading pre-R7 files)
			if (master != NULL)
			{
				master->MakeRefByID(FOREVER, 0, rtarg);
				if (rtarg != NULL)
				{
...
// End of PreR7 items
	...
	if (master != NULL)
	{
		*(ReactionMaster*)master = dummyMaster; // update the master if you actually found a preR7 master.
		MakeRefByID(FOREVER, 0, master);
	...
	if (anim->SuperClassID()==BASENODE_CLASS_ID)
	{
		...
		for(int x=0; x<dep.nodes.Count(); x++)
		{
			for(int i=0; i<dep.nodes[x]->NumSubs(); i++)
			{
				...
				{
					if (!(assignReactObj((INode*)dep.nodes[x], i))) return FALSE;
				}
			}
		}
	}
...
float Reactor::getCurFloatValue(TimeValue t)
{
	...
	if (GetMaster() != NULL) {
		if (c = GetControlInterface(GetMaster()->client->SubAnim(GetMaster()->subnum)))
			{
			c->GetValue(t, &f, FOREVER);
			return f;
			}
...
void Reactor::Update(TimeValue t)
{
	if (!ivalid.InInterval(t))
	{
		...
		if (GetMaster()!=NULL)
		{
			...
			}else if (GetControlInterface(GetMaster()->client->SubAnim(GetMaster()->subnum))) {
				switch (getReactionType())
				{
				case FLOAT_VAR:
					GetControlInterface(GetMaster()->client->SubAnim(GetMaster()->subnum))->GetValue(t, &f, ivalid);
					break;
				case VECTOR_VAR:
					GetControlInterface(GetMaster()->client->SubAnim(GetMaster()->subnum))->GetValue(t, &p, ivalid);
					break;
				case SCALE_VAR:
					GetControlInterface(GetMaster()->client->SubAnim(GetMaster()->subnum))->GetValue(t, &s, ivalid);
					break;
				case QUAT_VAR:
					GetControlInterface(GetMaster()->client->SubAnim(GetMaster()->subnum))->GetValue(t, &q, ivalid);
					break;
				}
			}
...
{ 
	...
	{
		case REFMSG_TARGET_DELETED:
			if (client->TestAFlag(A_BEING_AUTO_DELETED))
			{
				client = NULL;
...
void ReactionMaster::CopyFrom(Reactor* r)
{
	...
	for(int i = 0; i < from->states.Count(); i++)
	{
		...
		for(int x = 0; x < states.Count(); x++)
		{
			...
			if (found)
			{
				// slave states and master states should have a one-to-one coorespondence when loading Pre R7 files
				// this is not garenteed to be true when fixing R7 beta files.  These may have some problems...
				if (r->slaveStates.Count () > i)
