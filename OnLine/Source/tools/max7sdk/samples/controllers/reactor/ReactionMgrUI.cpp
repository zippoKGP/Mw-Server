...
{
	MyEnumProc dep;             
	curve->EnumDependents(&dep);

	for(int x=0; x<dep.nodes.Count(); x++)
	{
		if (dep.nodes[x]->ClassID() == CURVE_CONTROL_CLASS_ID)
		{
			MyEnumProc dep2;             
			curve->EnumDependents(&dep2);

			for(int i=0; i<dep2.nodes.Count(); i++)
			{
				Reactor* r = (Reactor*)GetIReactorInterface(dep2.nodes[i]);
				if (r != NULL)
					return r;
			}
		}
...
ReactionDlg::ReactionDlg(IObjParam *ip, HWND hParent)
{
	...
	theHold.Suspend();
	MakeRefByID(FOREVER,0,GetReactionManager());
	theHold.Resume();
...
bool ReactionDlg::AddSlaveRow(HWND hList, ReactionListItem* listItem, int itemID)
{
	...
	if (buf.Length() > 0)
	{
		...
		slaveItem.cchTextMax = _tcslen(buf2);
...
void ReactionDlg::UpdateReactionList()
{
	...
	for (int i=0; i<GetReactionManager()->ReactionCount(); i++) {
		...
		mainItem.cchTextMax = _tcslen(buf);
	...
	if (set != NULL)
	{
		...
		if (master != NULL)
		{
			for (int i=0; i<master->states.Count(); i++) 
			{
				...
				item.cchTextMax = _tcslen(state->name);
				if (listItem->IsExpandable())
				{
...
void ReactionDlg::UpdateCurveControl()
{
	...
	for (int x = 0; x < reactors.Count(); x++)
	{
		...
		if (r->iCCtrl != NULL && r->isCurveControlled)
		{
	...
	{
		iCCtrl->MakeRefByID(FOREVER, i, curves[i]);
	}
...
void ReactionDlg::UpdateButtons()
{
	...
	else
	{
		...
		if (slaveItems.Count() > 0)
		{
			...
			{
				if (slaveItems[i]->Slave()->GetMaster() == NULL)
				{
					createStates->Enable(FALSE);
					newState->Enable(FALSE);
					aborted = true;
...
void RemoveSlaveFromScene(Reactor* r)
{
	...
	MyEnumProc dep;             
	((ReferenceTarget*)r)->EnumDependents(&dep);
...
{
	ReactionDlg *dlg = (ReactionDlg*)GetWindowLongPtr(hWnd,GWLP_USERDATA);
	BOOL bHandled = FALSE;
	BOOL result = FALSE;
...
static LRESULT CALLBACK SplitterControlProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	ReactionDlg *dlg = (ReactionDlg*)GetWindowLongPtr(GetParent(hWnd),GWLP_USERDATA);
	BOOL bHandled = FALSE;
	BOOL result = FALSE;
...
static INT_PTR CALLBACK ReactionDlgProc(
	...
	switch (msg) {
		case WM_INITDIALOG:
			...
			HWND hList = GetDlgItem(hWnd, IDC_REACTION_LIST);
			SetWindowLongPtr(hList,GWLP_USERDATA,lParam);
			dlg->reactionListWndProc = (WNDPROC)SetWindowLongPtr(hList, GWLP_WNDPROC, (LONG_PTR)(ReactionListControlProc));
			
			hList = GetDlgItem(hWnd, IDC_STATE_LIST);
			SetWindowLongPtr(hList,GWLP_USERDATA,lParam);
			dlg->stateListWndProc = (WNDPROC)SetWindowLongPtr(hList, GWLP_WNDPROC, (LONG_PTR)(StateListControlProc));

			HWND hSplitter = GetDlgItem(hWnd, IDC_SPLITTER1);
			dlg->splitterWndProc = (WNDPROC)SetWindowLongPtr(hSplitter, GWLP_WNDPROC, (LONG_PTR)(SplitterControlProc));

			hSplitter = GetDlgItem(hWnd, IDC_SPLITTER2);
			SetWindowLongPtr(hSplitter, GWLP_WNDPROC, (LONG_PTR)(SplitterControlProc));
				...
				case LVN_ITEMCHANGED:
					{
						...
						if (isSelected != wasSelected && pListView->uOldState != 0x77d454b4)
						{
		...
		case WM_DESTROY:
			{
			HWND hList = GetDlgItem(hWnd, IDC_REACTION_LIST);
			SetWindowLongPtr(hList,GWLP_USERDATA,NULL);
			SetWindowLongPtr(hList, GWLP_WNDPROC, (LONG_PTR)dlg->reactionListWndProc);
			
			hList = GetDlgItem(hWnd, IDC_STATE_LIST);
			SetWindowLongPtr(hList,GWLP_USERDATA,NULL);
			SetWindowLongPtr(hList, GWLP_WNDPROC, (LONG_PTR)dlg->stateListWndProc);

			hList = GetDlgItem(hWnd, IDC_SPLITTER1);
			SetWindowLongPtr(hList, GWLP_WNDPROC, (LONG_PTR)dlg->splitterWndProc);
			...
			case REACTORFLOAT:
				...
				for(int x=0; x<dep.nodes.Count(); x++)
				{
					for(int i=0; i<dep.nodes[x]->NumSubs(); i++)
					{
						...
						if (n == r)
						{
							dim = dep.nodes[x]->GetParamDimension(i);
...
{ 
	...
	else if (owner != NULL)
	{
		TSTR buf = _T("");
		MyEnumProc dep; 
		owner->EnumDependents(&dep);

		for(int x=0; x<dep.nodes.Count(); x++)
		{
			for(int i=0; i<dep.nodes[x]->NumSubs(); i++)
			{
				ReferenceMaker* parent = dep.nodes[x];
