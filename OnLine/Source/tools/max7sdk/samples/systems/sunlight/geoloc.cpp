        ...
        if (NULL != pEntry)
        {
            for (int j=0; j<=LAST_MAP_NAME; ++j)
            {
                ...
                if (hMap != INVALID_HANDLE_VALUE)
                {
                    ...
                    DWORD bytesRead;
                    double latitude=0,longitude=0,width=0;
                    ...
                    if (  (-pEntry->longitude > (longitude - width / 2.)) //signs are reversed for longitude
                        &&(-pEntry->longitude < (longitude + width / 2.)) //signs are reversed for longitude
                        &&( pEntry->latitude  > (latitude - height / 2.))
                        &&( pEntry->latitude  < (latitude + height / 2.)))
                    {
                        mapIndex = j;
                        CloseHandle(hMap);
