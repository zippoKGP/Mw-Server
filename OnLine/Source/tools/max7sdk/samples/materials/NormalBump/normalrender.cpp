...
Point3 Gnormal::EvalNormalPerturb(ShadeContext& sc)
{
	...
	case TANGENTUV_SPACE: {
			...
			//initialize the tangent basis vectors, if necessary
			VNormalMgr* vnormalMgr = GetVNormalMgr();
			if( !vnormalMgr->ValidTangentBasis( sc.NodeID(), mapChannel ) )
				vnormalMgr->InitTangentBasis( sc, mapChannel );
