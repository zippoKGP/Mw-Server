...
void DxStdMtl2::Draw()
{
	...
	int w =0, h =0;
	myGWindow->GetWindowDimension(w,h);
	pd3dDevice->CreateStateBlock(D3DSBT_ALL ,&state);
	SetRenderStates();
...
bool DxStdMtl2::CreateAndLoadEffectData()
{
	if(IsDxMaterialEnabled(map))
	{
		...
		for(int i=0; i< textureCache.Count();i++)
		{
			textureCache[i]->SetParamHandle(NULL);
		}
		UpdateSceneLights();
		D3DCAPS9	Caps;
		int numberOfPSinst;
		...
		numberOfPSinst = Caps.PS20Caps.NumInstructionSlots;
		...
		else if (Caps.PixelShaderVersion < D3DPS_VERSION(4,0))
		{
			code = IHLSLCodeGenerator::PS_2_0;
		}
		else
...
void DxStdMtl2::PatchInLightNodes()
{
	for(int i = 0; i< sceneLights.Count(); i++)
	{
		...
		{
			LightElement * le = (LightElement*)elementContainer.GetElementByType(j, EffectElements::kEleLight);
...
void DxStdMtl2::LoadTextureData(IHLSLCodeGenerator * codeGen)
{
	...
	{
		bool bBump;
		TextureElement * texEle = textureCache[i];
...
void DxStdMtl2::NotificationCallback(void* param, NotifyInfo* info) {
	...
	if(info->intcode == NOTIFY_SCENE_ADDED_NODE || info->intcode == NOTIFY_SCENE_PRE_DELETED_NODE)
	{
		if(newNode)
		{
			if(!inHold)
			{
				ObjectState Os   = newNode->EvalWorldState(GetCOREInterface()->GetTime());
	...
	if(info->intcode == NOTIFY_SCENE_UNDO || info->intcode == NOTIFY_SCENE_REDO)
	{
		if(shader){
			shader->UpdateSceneLights();
			shader->CreateAndLoadEffectData();
...
void DxStdMtl2::SetTextureParam(D3DXHANDLE handle, TCHAR * name,TCHAR * uiName, TCHAR * filename, MaxSemantics semantic, bool mappingEnabled, int mappingChannel)
{
	...
	if(!EffectParam)
	{
		EffectParam  = new TextureElement(handle,filename);
		EffectParam->SetSemantic(semantic);
		EffectParam->SetParamHandle(handle);
		textureCache.Append(1,&EffectParam);
		return;
	}
	EffectParam->SetSemantic(semantic);
	EffectParam->SetParamHandle(handle);
...
ILightingData * DxStdMtl2::GetLightingData(D3DXHANDLE handle)
{
	...
	{
		LightElement * le = (LightElement*)elementContainer.GetElementByType(i,EffectElements::kEleLight);
...
TextureElement * DxStdMtl2::FindTextureElement(TCHAR * name)
{
	for(int i=0; i< textureCache.Count();i++)
	{
		if(_tcscmp(textureCache[i]->GetMapName().data(), name)==0)
...
TextureElement::TextureElement(D3DXHANDLE handle, TCHAR * filename)
{
