...
{	
	...
	// aszabo|sep.15.04|#596889 
	if (!vpt->IsGridVisible())
		return;
...
#else //!GAME_VER
		// aszabo|sep.15.04|#596889|The grid spacing shouldn't be zero, but if it is, it can lead to infinite loops
		DbgAssert(grid > 0.0f || grid < 0.0f);
		if (grid > 0.0f || grid < 0.0f)
		{
			...
			// Find all snap points on the right side (positive x) of the intsnapped
			while (CheckPotentialHit(&world_snapped,0, fp)) 
			{
				got_one = TRUE;
				RecordNewIntSubHit(world_snapped, tmconst, curIntsnapped, grid);

				// Go down the vertical grid line
				curIntsnapped.y -= grid;
				FindVerticalIntSubHit(tmconst, curIntsnapped, fp, (-grid));

				// Go up the vertical grid line
				curIntsnapped.y = intsnapped.y + grid;
				FindVerticalIntSubHit(tmconst, curIntsnapped, fp, grid);

				// Go to the right of the original vertival grid line
				curIntsnapped.y = intsnapped.y;
				curIntsnapped.x += grid;
				world_snapped = tmconst * curIntsnapped;
			} 
			// Find all snap points on the left side (negative x) of the intsnapped
			curIntsnapped.y = intsnapped.y;
			curIntsnapped.x = intsnapped.x - grid;
			world_snapped = tmconst * curIntsnapped;
			while (CheckPotentialHit(&world_snapped,0, fp)) 
			{
				got_one = TRUE;
