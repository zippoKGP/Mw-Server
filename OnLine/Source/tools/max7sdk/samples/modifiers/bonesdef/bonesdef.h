		...
		void BuildBlurData(int whichBone)
		{
			...
			else if (mnEdges.Count() != 0)
			{
				...
				for (int i = 0; i < mnEdges.Count(); i++)
				{
					...
					int numberOfBones = VertexData[v1]->d.Count();
					for (int k = 0; k <numberOfBones; k++)
					{
						int boneID = VertexData[v1]->d[k].Bones;
						if (boneID == whichBone)
						{
							//found a vertex with some weight
							blurredWeights[v2] +=  VertexData[v1]->d[k].normalizedInfluences;
							weightCounts[v2] += 1;
						}
					}
					...
					numberOfBones = VertexData[v1]->d.Count();
					for (int k = 0; k <numberOfBones; k++)
					{
						int boneID = VertexData[v1]->d[k].Bones;
						if (boneID == whichBone)
