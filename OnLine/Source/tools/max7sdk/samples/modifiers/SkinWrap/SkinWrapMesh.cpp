...
BOOL MeshDeformParamsMapDlgProc::DlgProc(
...
BOOL MeshDeformParamsAdvanceMapDlgProc::DlgProc(
		TimeValue t,IParamMap2 *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
...
//	potentialEdges.SetSize(msh->numVerts);
	...
	for (vid = 0; vid < msh->numVerts; vid++)
		{
	...
	else
	{
		if (wrapIndex >= localData->adjEdgeList.Count())
		{
			for (i = 0; i < msh->numVerts; i++)
			{
				...
				for (j =0; j < localData->paramData.Count(); j++)
				{
...
void MeshDeformPW::BuildParamData(Object *obj, LocalMeshData *localData,int wrapIndex, INode *node,Matrix3 meshTM,Matrix3 baseTM, float threshold)
	{
//put our base mesh data into mesh space
	Matrix3 tmFromBaseToMesh = baseTM * Inverse(meshTM);
...
void MeshDeformPW::BuildParamDataForce(Object *obj, LocalMeshData *localData,int wrapIndex, INode *node,Matrix3 meshTM,Matrix3 baseTM, float threshold, BitArray pointsToProcess)
	{
...
void MeshDeformPW::ModifyObject(TimeValue t, ModContext &mc, ObjectState * os, INode *incnode) 
{
	...
	for (wrapIndex = 0; wrapIndex < pblock->Count(pb_meshlist); wrapIndex++)
	{
		INode *node = NULL;
		pblock->GetValue(pb_meshlist,t,node,iv,wrapIndex);
		...
		if (meshos.obj) 
		{
			...
			Matrix3 meshTM = node->GetObjTMAfterWSM(t,&iv);//node->GetObjectTM(t, &iv);
			wrapMeshData[wrapIndex]->currentMeshTM = meshTM;
...
//			Matrix3 otm = node->GetObjectTM(t, &iv);
//			Matrix3 btm = node->GetObjTMBeforeWSM(t, &iv);
//			Matrix3 atm = node->GetObjTMAfterWSM(t, &iv);

			Matrix3 meshTM = node->GetObjTMAfterWSM(t, &iv);;//node->GetObjectTM(t, &iv);
			wrapMeshData[wrapIndex]->initialMeshTM = meshTM;

			//if no node bail
			if ((node != NULL) && (wrapMeshData[wrapIndex]->mesh))
			{
				BuildParamData(os->obj,localData,wrapIndex,node,wrapMeshData[wrapIndex]->initialMeshTM,localData->initialBaseTM,threshold);
...
//				DebugPrint("Point with no weight %d\n",i);
		...
		if (unassignedFaceFound)
		{
			...
			while (facesToProcess.NumberSet() > 0)
			{
				...
				//if no node bail
					if ((node != NULL) && (wrapMeshData[wrapIndex]->mesh))
						BuildParamDataForce(os->obj,localData,wrapIndex,node,wrapMeshData[wrapIndex]->initialMeshTM,baseTM,tThreshold,facesToProcess);
				}				
				facesToProcess.ClearAll();
			...
			if (ip ) 
			{
				...
				for (int i = 0; i < ct; i++)
				{
					if (wrapMeshData[wrapIndex]->selectedVerts[i])
					{
						...
						gw->marker(&wrapMeshData[wrapIndex]->currentVertexTMs[i].GetTrans(),SM_DOT_MRKR);
					}
					else if (showVerts)
					{
...
//								axis[0] = Point3(0.0f,0.0f,0.0f);
		...
		if ((mc->localData) && (ip && ip->GetSubObjectLevel() == 1) )
		{
			...
			if (engine == 0)
			{
				...
				for (i = 0; i < localData->localPoint.Count(); i++)
				{
...
//free our lookup list
	...
	//loop through
			if (wrapMeshData[wrapIndex]->selectedVerts[i])
			{
				...
				vertList.ClearAll();
				for (j = 0; j < ld->paramData.Count(); j++)
				{
...
//					int nodeIndex = ld->paramData[j].whichNode;
					if (faceList[faceIndex] /*&& (nodeIndex==wrapIndex)*/)
					{
						vertList.Set(j,TRUE);
					}
				}
				int ct = 0;
				for (j = 0; j < vertList.GetSize(); j++)
				{
					if (vertList[j])
					{
...
void MeshDeformPW::BakeControlPoints()
{
	...
	for (int wrapIndex = 0; wrapIndex < pblock->Count(pb_meshlist); wrapIndex++)
	{
		...
		float *pFalloff,*pDistance;
		int *pEngine, *pFaceLimit;
		pFalloff = new float;
		pDistance = new float;
		pEngine = new int;
		pFaceLimit = new int;
		...
		*pCt = ct;
		...
		initialDataTemp = new InitialTMData[ct];
		for (int i = 0; i < ct; i++)
