...
BOOL PatchDeformParamsMapDlgProc::DlgProc(
...
//split the patches into 10x10 chunks
	...
	for (i = 0; i < numPatches; i++)
		...
		if (ip)
			{
			TSTR name;
			name.printf("Completed %3.0f%%",(float) i / numPatches *100.0f);
		...
		if ( (usedPatches[i]) && (p->type == PATCH_QUAD))
			...
			for (int j = 0; j < pointCount; j++)
				...
				if ((ip) && ((j%10) == 0))
					{
					TSTR name;
					name.printf("Completed %3.0f%%(%d/%d)",(float) i / numPatches *100.0f,j,pointCount);
					SetWindowText(GetDlgItem(hWnd,IDC_STATUS),name);
					}
				if (closestPoint[j] == i)
					{
...
void PatchDeformPW::ModifyObject(TimeValue t, ModContext &mc, ObjectState * os, INode *incnode) 
{
	...
	if ((patchos.obj) && (localData->selfNode))
		...
		if ((!patchValid) && (ip))
			{
			TSTR name;
			name.printf("Error TriPatch Found");
			SetWindowText(GetDlgItem(hWnd,IDC_STATUS),name);
			}
