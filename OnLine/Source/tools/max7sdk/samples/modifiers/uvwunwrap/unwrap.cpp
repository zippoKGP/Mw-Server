...
static INT_PTR CALLBACK UnwrapRollupWndProc(
	...
	switch (msg) {
		case WM_INITDIALOG:
			...
			CheckDlgButton(hWnd,IDC_NOSEAM,FALSE);
			...
			{
				CheckDlgButton(hWnd,IDC_NOSEAM,TRUE);
			}
				...
				case IDC_THICKSEAM:
					{
						...
						BOOL noSeams = IsDlgButtonChecked(hWnd,IDC_NOSEAM);
...
//	mod->cont      = cont;
	...
	mod->TVMaps.geomPoints.SetCount(0);// = TVMaps.geomPoints;
...
*/
	...
	if (rect.w() < 600)
		...
		MoveWindow(GetDlgItem(hWnd,IDC_UNWRAP_FILTER_TOOLBAR),
			220+12, ys-5, filterSize, TOOL_HEIGHT, TRUE);
#else
		MoveWindow(GetDlgItem(hWnd,IDC_UNWRAP_FILTER_TOOLBAR),
			220+12, ys-5, filterSize, TOOL_HEIGHT, TRUE);
#endif // APL_DBCS
		MoveWindow(GetDlgItem(hWnd,IDC_UNWARP_VIEW_TOOLBAR),
			440+12, ys-5, viewSize, TOOL_HEIGHT, TRUE);
...
#ifdef APL_DBCS		// --hide.
		MoveWindow(GetDlgItem(hWnd,IDC_UNWRAP_FILTER_TOOLBAR),
			rect.w()-380+12, ys-5, filterSize, TOOL_HEIGHT, TRUE);
#else
		MoveWindow(GetDlgItem(hWnd,IDC_UNWRAP_FILTER_TOOLBAR),
			rect.w()-380+12, ys-5, filterSize, TOOL_HEIGHT, TRUE);
#endif // APL_DBCS
		MoveWindow(GetDlgItem(hWnd,IDC_UNWARP_VIEW_TOOLBAR),
			rect.w()-123-32+12, ys-5, viewSize, TOOL_HEIGHT, TRUE);
