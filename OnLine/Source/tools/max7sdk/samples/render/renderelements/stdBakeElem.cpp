...
static ParamBlockDesc2 diffuseBakeParamBlk(PBLOCK_NUMBER, _T("Diffuse TextureBake Parameters"), 0, &diffuseBakeCD, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
	...
	BaseBakeElement::fileName, _T("fileName"), TYPE_STRING, 0, IDS_FILE_NAME,
		p_default, "",
		end,
	BaseBakeElement::fileNameUnique, _T("filenameUnique"), TYPE_BOOL, 0, IDS_FILENAME_UNIQUE,
		p_default, FALSE,
		end,
	BaseBakeElement::fileType, _T("fileType"), TYPE_STRING, 0, IDS_FILE_TYPE,
		p_default, "",
		end,
...
void DiffuseBakeElement::TranslateParameters(imrShader* shader, TimeValue t, Interval& valid) {
...
void SpecularBakeElement::TranslateParameters(imrShader* shader, TimeValue t, Interval& valid) {
...
void LightBakeElement::TranslateParameters(imrShader* shader, TimeValue t, Interval& valid) {
...
void ReflectRefractBakeElement::TranslateParameters(imrShader* shader, TimeValue t, Interval& valid) {
...
void ShadowBakeElement::TranslateParameters(imrShader* shader, TimeValue t, Interval& valid) {
...
void NormalsBakeElement::PostIllum(ShadeContext& sc, IllumParams& ip)
{
	...
	if( !render_projectionOn ) { //normals, no projection
		...
		default: //default is tangent space
			Point3 n = normal;
			Point3 t[3];
			workingModelInfo->TangentBasisVectors( &(t[0]), render_mapChannel  );
			t[2] = workingModelInfo->GetOrigNormal();

			//convert the normal to tangent space using the texture basis vectors
			normal.x = DotProd( t[0], n );
			normal.y = DotProd( t[1], n );
			normal.z = DotProd( t[2], n );
...
void NormalsBakeElement::TranslateParameters(imrShader* shader, TimeValue t, Interval& valid) {
...
void CompleteBakeElement::TranslateParameters(imrShader* shader, TimeValue t, Interval& valid) {
...
void BlendBakeElement::TranslateParameters(imrShader* shader, TimeValue t, Interval& valid) {
...
void AlphaBakeElement::TranslateParameters(imrShader* shader, TimeValue t, Interval& valid) {
...
void HeightBakeElement::TranslateParameters(imrShader* shader, TimeValue t, Interval& valid) {
