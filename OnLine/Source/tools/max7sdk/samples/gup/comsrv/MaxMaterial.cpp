...
static void MtlRefAddedProc(void *param, NotifyInfo *info) {
	...
	else if(rMaker->IsSubMtl() && rMaker->GetReference(REFIDX_MTL))
	{
		...
		rMaker->mpMtl->EnumDependents(&proc);
		if(proc.found)
		{
			PartID PartID = PART_ALL;
			rMaker->NotifyRefChanged(FOREVER, rTarg, PartID, REFMSG_REF_ADDED);
...
STDMETHODIMP CMaxMaterial::saveXML(IUnknown* pXMLNode, VARIANT_BOOL noPrompts)
{
	...
	CComPtr<IXMLDOMNode> materials, material, child;
...
void CMaxMaterial::CheckReference(bool added, bool removed)
{
	...
	if(mpCollection)
	{
		...
		if(removed)
		{
			...
			else if((mcNodeRefs == 0 && cNodeRefsLast)
					|| (mcMaxRefs == 0 && cMaxRefsLast))
		...
		if (added)
		{
			...
			// then this material may need to be moved to another dynamic palette.
			if((mcNodeRefs  && cNodeRefsLast == 0)
					|| (mcMaxRefs && cMaxRefsLast == 0))
...
void CMaxMaterial::setMtl(Mtl *pMat)
{
	...
	if(GetReference(REFIDX_MTL))
	{
		ATLASSERT(0);//Don't allow this
		res = ReplaceReference(REFIDX_MTL, pMat);
	}
	else
		res = MakeRefByID(FOREVER, REFIDX_MTL, pMat);
...
public:
	void proc(ReferenceMaker *rm) {	rm->FreeAllBitmaps(); }
};

void RecursiveFreeAllBitmaps(MtlBase *mb)
{
	FreeAllMapsRefEnum freeEnum;
	EnumRefHierarchy(mb, freeEnum);
}
...
#if 1
	//test code to time marshaller
	//create a huge white bitmap
	if(sz == 3)
	{
		mPSByteCount = BIG_HUNK;
		mPSByteCache = new UBYTE[mPSByteCount];
		ZeroMemory(mPSByteCache, mPSByteCount);
...
INode *DropNode(DWORD hWnd, POINT point)
{
	POINT screen = point;
...
STDMETHODIMP CMaxMaterial::OnDrop(SHORT nFlag, DWORD hWnd, DWORD point, DWORD dwKeyState)
{
	ATLASSERT(nFlag);
...
//			GetCursorPos(&pt);
			...
			if(1)//1/30/03 allow updating the scene with a drop in space
			{
				...
				else //no node, possibly put the material
				{
					...
					TSTR name = mpMtl->GetName();
					oldMtl = pInt->FindMtlNameInScene(name);
					if(oldMtl && oldMtl!=mpMtl)
					{
						//3/3/03 This case would not remove from current scene tab
						RecursiveAddMtl(oldMtl, mtls);
...
#endif //DROPFROMSCRATCH
										pInt->PutMaterial(mpMtl, oldMtl, 0, this);//has built-in undo support
