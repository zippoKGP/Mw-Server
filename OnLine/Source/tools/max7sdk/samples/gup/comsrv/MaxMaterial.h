...
//		bool isValid(){return mValid;}
		...
		void Invalidate();
		unsigned char *getBytes(){return mPSByteCache;}
		void Init(BSTR path);
	private:
		int mPSByteCount;
//		bool mValid;
...
//		CComBSTR mBSTRPSpath;
		unsigned char * mPSByteCache; //keep a cache of the bytes
	} mPSCache;
	GUID mID;
	Mtl* mpMtl;
	DWORD mdwKey;//mpMtl as a DWORD, we may need this key after the material has self-deleted
...
public:
	STDMETHOD(OnDrop)(SHORT nFlag, DWORD hWnd, DWORD point, DWORD dwKeyState);
