...
BOOL PAOperatorDisplayDlgProc::DlgProc(TimeValue t, IParamMap2 *map, HWND hWnd, UINT msg,
...
void PAOperatorDisplayDlgProc::UpdateDlg( HWND hWnd, IParamBlock2* pblock )
{
	...
	ReleaseIColorSwatch(color);
	for(i=2; i<kNumGradientValues; i++) {
	...
	int showControl;
	for(i=0; i<kNumGradientValues; i++) {
