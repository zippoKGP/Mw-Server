...
class IGameExporter : public SceneExport {
	public:
		...
		void DumpGenericFace(IGameMesh * gm,FaceEx *fe, CComPtr<IXMLDOMNode> faceData, bool smgrp, bool n, bool vc, bool tv);
...
BOOL CALLBACK IGameExporterOptionsDlgProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam) {
	...
	switch(message) {
		case WM_INITDIALOG:
			...
			//Versioning
			TCHAR Title [256];
            _stprintf(Title,"IGame Exporter using IGame version %.3f",exp->version);
...
void IGameExporter::DumpListController(IGameControl * sc,CComPtr<IXMLDOMNode> prsNode)
{
		...
		for(int i=0;i<subNum;i++)
	{
		IGameKeyTab SubCont;
			...
			ExportPositionControllers(posListNode,sub);
...
void IGameExporter::DumpGenericFace(IGameMesh * gM,FaceEx *fe, CComPtr<IXMLDOMNode> faceData, bool smgrp, bool n, bool vc, bool tv)
{
	TSTR data;
	FaceEx *f = fe;
	...
	if(exportNormals && n && !exportNormalsPerFace)
	{
		prop =NULL;
	...
	if(exportNormalsPerFace)
	{
		CComPtr <IXMLDOMNode> perFaceNorm = NULL;
		CreateXMLNode(pXMLDoc,faceData,_T("VertexNormals"),&perFaceNorm);
		for(int vv=0; vv<3; vv++)
		{
			CComPtr <IXMLDOMNode> normals = NULL;
			CreateXMLNode(pXMLDoc,perFaceNorm,_T("Normal"),&normals);
			Point3 n;
			if(gM->GetNormal(f,vv, n))
			{
				buf.printf("%f %f %f\n",n.x,n.y,n.z);
...
void IGameExporter::DumpMesh(IGameMesh * gm,CComPtr<IXMLDOMNode> geomData)
{
	...
	if(exportNormals && !exportNormalsPerFace)
	{
		// dump Normals
	...
	if(exportFaceSmgp)
	{
		...
		for (int x=0;x<smGrps.Count();x++)
		{
			...
			for(int f=0;f<smFace.Count();f++)
			{
				...
				DumpGenericFace(gm,gm->GetFace(smFace[f]),norm,false,nSet,vcSet,tvSet);
			}
	...
	else
	{
		...
		for(int f=0;f<numFaces;f++)
		{
			...
			data.printf("%d",f);
			AddXMLAttribute(norm,_T("index"),data.data());
			DumpGenericFace(gm, gm->GetFace(f),norm,true,nSet,vcSet,tvSet);
