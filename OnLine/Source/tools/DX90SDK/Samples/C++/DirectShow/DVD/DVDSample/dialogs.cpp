...
int CAngleDlg::MakeAngleList(HWND hDlg, int iListID)
{
    ...
    {
        wsprintf(szAngle, TEXT("Angle %u\0"), ulAngle);
...
BOOL CALLBACK CChapterDlg::ChapterDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        case WM_INITDIALOG:
        {
            ...
            // set default value
            TCHAR buf[10];
            wsprintf(buf, TEXT("%u\0"), pThis->m_ulChapter); 
...
BOOL CALLBACK CTitleDlg::TitleDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        case WM_INITDIALOG:
        {
            ...
            // set default values
            TCHAR buf[10];
            wsprintf(buf, TEXT("%u\0"), pThis->m_ulTitle); 
            SetDlgItemText(hDlg, IDC_PLAYCHAPTER, buf);
            wsprintf(buf, TEXT("%u\0"), pThis->m_ulChapter); 
            SetDlgItemText(hDlg, IDC_PLAYCHAPTER, buf);
...
BOOL CALLBACK CTimeDlg::TimeDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        case WM_INITDIALOG:
        {
            ...
            // set default values
            TCHAR buf[10];
            wsprintf(buf, TEXT("%u\0"), pThis->m_Time.bHours); 
            SetDlgItemText(hDlg, IDC_HOURS, buf);
            wsprintf(buf, TEXT("%u\0"), pThis->m_Time.bMinutes); 
            SetDlgItemText(hDlg, IDC_MINUTES, buf);
            wsprintf(buf, TEXT("%u\0"), pThis->m_Time.bSeconds); 
