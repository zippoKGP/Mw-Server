...
class CMyD3DApplication : public CD3DApplication
{
    ...
    BOOL                m_bCantDoSingleStrip; // Single strip would be too many primitives
...
HRESULT CMyD3DApplication::FrameMove()
{
    ...
    D3DXVECTOR3 vFrom( 0, 0,-3.7f*m_fObjectRadius * m_cObjectsPerSide);
...
HRESULT CMyD3DApplication::DrawMeshData(SMeshData *pMeshData)
{
    ...
    for( DWORD iMaterial=0; iMaterial < m_dwNumMaterials; iMaterial++ )
    {
        ...
        else  // drawing strips
        {
            ...
            m_pd3dDevice->SetStreamSource(0, pMeshData->m_pVertexBuffer, 0, cBytesPerVertex);

            if(m_bShowSingleStrip)
            {
                if( !m_bCantDoSingleStrip )
                {
                    m_pd3dDevice->SetIndices( pMeshData->m_rgStripData[iMaterial].m_pStrips );

                    hr = m_pd3dDevice->DrawIndexedPrimitive( D3DPT_TRIANGLESTRIP, 0,
            ...
            {
                m_pd3dDevice->SetIndices( pMeshData->m_rgStripData[iMaterial].m_pStripsMany );
...
HRESULT CMyD3DApplication::Render()
{
    ...
    if( SUCCEEDED( m_pd3dDevice->BeginScene() ) )
    {
        ...
        if( m_bShowSingleStrip && m_bCantDoSingleStrip )
        {
            lstrcpy( strInfo, TEXT("Couldn't draw to single strip -- too many primitives") );
            m_pFont->DrawText( 2, 60, D3DCOLOR_ARGB(255,255,0,0), strInfo);
        }
    ...
    )
{
    ...
    if( FAILED( hr = DXUtil_FindMediaFileCb( strMesh, sizeof(strMesh), m_strMeshFilename ) ) )
        goto End;
    ...
    for( i=0; i<m_dwNumMaterials; i++ )
    {
        ...
        {
            DXUtil_FindMediaFileCb( strPath, sizeof(strPath), d3dxMaterials[i].pTextureFilename );
    ...
    if( SUCCEEDED(hr) )
    {
        ...
        if( SUCCEEDED(hr) )
        {
            hr = D3DXComputeBoundingSphere( (D3DXVECTOR3*)pVertices, pMeshSysMem->GetNumVertices(),
    ...
    )
{
    ...
    for (iMaterial = 0; iMaterial < m_dwNumMaterials; iMaterial++)
    {
        ...
        if( primCount > m_d3dCaps.MaxPrimitiveCount )
        {
            m_bCantDoSingleStrip = TRUE;
        }
