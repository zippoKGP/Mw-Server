...
{
    BOOL bRangeFogSupported = TRUE;
    if( (m_dwCreateFlags & D3DCREATE_SOFTWARE_VERTEXPROCESSING) == 0 &&
        (m_d3dCaps.RasterCaps & D3DPRASTERCAPS_FOGRANGE) == 0 )
    {
        bRangeFogSupported = FALSE;
    }
    GetDlgItem(IDC_RANGEBASEDFOG)->EnableWindow(bRangeFogSupported);
...
//----------------------------------------------------------------------------
HRESULT CAppForm::GenerateTerrainDisk( LPDIRECT3DDEVICE9 pd3dDevice, DWORD dwNumSegments,
                                       FLOAT fScale )
{
    HRESULT hr;
    ...
    hr = pd3dDevice->CreateVertexBuffer( m_dwNumTerrainVertices*sizeof(FOGVERTEX),
                                         ...
                                         D3DPOOL_MANAGED, &m_pTerrainVB, NULL );
    if( FAILED(hr) )
        return hr;

    FOGVERTEX* pVertices = NULL;
    hr = m_pTerrainVB->Lock( 0, 0, (void**)&pVertices, 0 );
    if( FAILED(hr) )
        return hr;
...
//----------------------------------------------------------------------------
HRESULT CAppForm::GenerateColumn( LPDIRECT3DDEVICE9 pd3dDevice, DWORD dwNumSegments,
                                  FLOAT fRadius, FLOAT fHeight )
{
    ...
    hr = pd3dDevice->CreateVertexBuffer( m_dwNumColumnVertices*sizeof(FOGVERTEX),
                                         D3DUSAGE_WRITEONLY, D3DFVF_FOGVERTEX,
                                         D3DPOOL_MANAGED, &m_pColumnVB, NULL );
    if( FAILED(hr) )
        return hr;

    FOGVERTEX* pVertices = NULL;
    hr = m_pColumnVB->Lock( 0, 0, (void**)(&pVertices), 0 );
    if( FAILED(hr) )
        return hr;
...
HRESULT CAppForm::Render()
{
    ...
    if( SUCCEEDED( m_pd3dDevice->BeginScene() ) )
    {
        ...
        // Draw the terrain
        m_pd3dDevice->SetFVF( D3DFVF_FOGVERTEX );
        m_pd3dDevice->SetStreamSource( 0, m_pTerrainVB, 0, sizeof(FOGVERTEX) );
        ...
        for( DWORD i=0; i<20; i++ )
        {
            ...
            m_pd3dDevice->SetFVF( D3DFVF_FOGVERTEX );
            m_pd3dDevice->SetStreamSource( 0, m_pColumnVB, 0, sizeof(FOGVERTEX) );
            m_pd3dDevice->DrawPrimitive( D3DPT_TRIANGLESTRIP,
...
{
    // Need to support at least vertex fog
    if( ( pCaps->RasterCaps & D3DPRASTERCAPS_FOGVERTEX ) == 0 )
        return E_FAIL;
