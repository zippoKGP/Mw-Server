...
VOID OnInitDialog( HWND hDlg )
{
    ...
    {
        DXTRACE_ERR_MSGBOX( TEXT("InitAudio"), hr );
    ...
    PostMessage( hDopplerSlider, TBM_SETRANGEMAX, TRUE, ConvertLogScaleToLinearSliderPosTo( DS3D_MAXDOPPLERFACTOR ) );
    PostMessage( hDopplerSlider, TBM_SETRANGEMIN, TRUE, ConvertLogScaleToLinearSliderPosTo( DS3D_MINDOPPLERFACTOR ) );

    PostMessage( hRolloffSlider, TBM_SETRANGEMAX, TRUE, ConvertLogScaleToLinearSliderPosTo( DS3D_MAXROLLOFFFACTOR ) );
    PostMessage( hRolloffSlider, TBM_SETRANGEMIN, TRUE, ConvertLogScaleToLinearSliderPosTo( DS3D_MINROLLOFFFACTOR ) );
...
VOID OnSliderChanged( HWND hDlg )
{
    ...
    HWND hHorzSlider = GetDlgItem( hDlg, IDC_HORIZONTAL_SLIDER );
    HWND hVertSlider = GetDlgItem( hDlg, IDC_VERTICAL_SLIDER );
    float fXRadius = fabsf( SendMessage( hHorzSlider, TBM_GETPOS, 0, 0 ) / 100.0f * ORBIT_MAX_RADIUS);
    float fYRadius = fabsf( SendMessage( hVertSlider, TBM_GETPOS, 0, 0 ) / 100.0f * ORBIT_MAX_RADIUS);
    if( fabsf(fXRadius - fYRadius) < 0.01f )
    {
        if( fabsf(fXRadius - fMaxDistance) < 0.01f )
        {
            // If you have a sound source orbiting around the 
            // listener in a circle of radius R, and you set the MaxDistance 
            // to R, the sound will mute and un-mute very frequently 
            // So if this happens then add a tiny fudge factor of 
            // 0.01f to fMaxDistance to prevent this
            // Note: This only applies to software 3D buffer since
            // MUTE3DATMAXDISTANCE only affects software buffers
            fMaxDistance += 0.01f;
        }
        if( fabsf(fXRadius - fMinDistance) < 0.01f )
        {
            // Same goes for the fMinDist case
            fMinDistance += 0.01f;
        }
    }
