                                                  ...
                                                  DMUS_PMSG* pPMsg )
{
    ...
    //  
    // This tool is sending the "echo" pmsges to different channel groups.
    // Channel Groups is a foreign concept to legacy MIDI, but is a wonderful
    // feature in DirectMusic. Doing this prevents typical MIDI note overlap
    // problems. While echoing other pmsg types is not required it is a good
    // idea, so all nuances of the original notes carry over to the echoed notes.
