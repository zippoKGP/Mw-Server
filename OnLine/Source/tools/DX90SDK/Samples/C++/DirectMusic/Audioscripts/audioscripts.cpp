        ...
        case WM_COMMAND:
            switch( LOWORD(wParam) )
            {
                ...
                case IDC_BUTTON_ROUTINE3:
                    ...
                    {
                        TCHAR pstrRoutine[] = TEXT("RoutineButtonX");
...
HRESULT OnInitDialog( HWND hDlg )
{
    ...
    // Set the default media path (something like C:\DXSDK\SAMPLES\MULTIMEDIA\MEDIA)
    // to be used as the search directory for finding DirectMusic content.
    TCHAR strMediaPath[512];
    DXUtil_GetDXSDKMediaPathCb( strMediaPath, sizeof(strMediaPath) );
    ...
    if( FAILED( hr = OnChangeScriptFile( hDlg, "ScriptDemoBasic.spt" ) ) )
    {
        DXTRACE_ERR( TEXT("OnChangeScriptFile"), hr );
        MessageBox( hDlg, "Error loading script.", 
                          "DirectMusic Sample", MB_OK | MB_ICONERROR );
    ...
    SendDlgItemMessage( hDlg, IDC_COMBO_SCRIPT, CB_SELECTSTRING, 
                        (WPARAM)-1, (LPARAM) "ScriptDemoBasic.spt" );
...
HRESULT OnChangeScriptFile( HWND hDlg, TCHAR* strFileName )
{
    ...
    if( FAILED( hr = UpdateVariables( hDlg ) ) )
        return DXTRACE_ERR_MSGBOX( TEXT("UpdateVariables"), hr );
...
//-----------------------------------------------------------------------------
// Name: OnCallRoutine()
// Desc: Calls the given script routine and checks for variable changes
//-----------------------------------------------------------------------------
HRESULT OnCallRoutine( HWND hDlg, TCHAR* strRoutine )
{
    HRESULT hr;
    ...
    if( FAILED( hr = g_pMusicScript->CallRoutine( strRoutine ) ) )
        return DXTRACE_ERR_MSGBOX( TEXT("CallRoutine"), hr );
...
// Desc: Validates the current UI for variables values
//-----------------------------------------------------------------------------
HRESULT UpdateVariables( HWND hDlg )
{
    LONG lVal = 0;
    ...
    if( FAILED( hr = g_pMusicScript->GetVariableNumber( TEXT("Variable1"), &lVal ) ) )
        return DXTRACE_ERR_MSGBOX( TEXT("GetVariableNumber"), hr );
...
//-----------------------------------------------------------------------------
// Name: SetVariable()
// Desc: Sets the value of the given script variable
//-----------------------------------------------------------------------------
HRESULT SetVariable( HWND hDlg, int nIDDlgItem )
{
    HRESULT hr;
    ...
    TCHAR pstrRoutine[] = TEXT("RoutineEditX");
                   
    pstrRoutine[_tcslen(pstrRoutine) - 1] = '1' + (nIDDlgItem - IDC_EDIT_VARIABLE1);

    hr = g_pMusicScript->SetVariableNumber( pstrVariable, iVal );
    if( FAILED(hr) )
        return DXTRACE_ERR_MSGBOX( TEXT("SetVariable"), hr );

    // This is a convenient place to notify the script about the variable change.
    // 'ScriptDemoBasic.spt' uses this routine call to update the current effect
    // 'ScriptDemoBaseball.spt' ignores this routine and instead uses a looping
    // segment to check for changes to the variables on each beat
    hr = OnCallRoutine(hDlg, pstrRoutine);
    if( FAILED(hr) )
        return DXTRACE_ERR_MSGBOX( TEXT("OnCallRoutine"), hr );
