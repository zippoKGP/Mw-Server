...
VOID OnInitDialog( HWND hDlg )
{
    ...
    if( FAILED( hr = g_pMusicManager->Initialize( hDlg, 128, DMUS_APATH_DYNAMIC_STEREO ) ) )
    {
    ...
    // Load default file
    TCHAR strFile[512];
    if( GetWindowsDirectory( strFile, MAX_PATH+1 ) == 0 )
        return;
    ...
    if( SUCCEEDED( hr = g_pMusicManager->CreateSegmentFromFile( &g_pSegment, g_strFileName, TRUE, FALSE ) ) )
    {
        g_lpFXManager->Initialize( g_pMusicManager->GetDefaultAudioPath(), TRUE );
...
HRESULT OnPlaySound( HWND hDlg )
{
    ...
    if( FAILED( hr = g_pMusicManager->CreateSegmentFromFile( &g_pSegment, g_strFileName, 
                                                              TRUE, bMidiFile ) ) )
    {
    ...
    g_lpFXManager->Initialize( g_pMusicManager->GetDefaultAudioPath(), FALSE );
...
VOID OnOpenSoundFile( HWND hDlg ) 
{
    static TCHAR strFileName[512] = TEXT("");
    static TCHAR strPath[512] = TEXT("");
