...
public:
	...
	// GetSurface should not be used as only IDirect3DSurface9 should be used. Instead, use GetSurface3D.
	IDirectDrawSurface *GetSurface();	// must release when done with returned surface (it's addref'd before returned)
	IDirect3DSurface9 *GetSurface3D();	// must release when done with returned surface (it's addref'd before returned)
...
private:
	...
	IDirect3DSurface9 *m_pSurface3D;
