...
int WINAPI WinMain( HINSTANCE hInst, HINSTANCE, LPSTR, int )
{
    ...
    HWND hDlg = CreateDialog( hInst, MAKEINTRESOURCE(IDD_KEYBOARD), NULL, MainDlgProc );
    ...
    while( GetMessage( &msg, NULL, 0, 0) )
    {
        if( g_pKeyboard )
        {
            // If the keyboard is acquired in non-exclusive mode, windows will 
            // continue to send key messages to the application. In dialog boxes, 
            // these messages interact with the dailog's controls if handled, and 
            // trigger the default beep sound if unhandled. This code filters
            // out undesired key messages if the keyboard is currently acquired.
