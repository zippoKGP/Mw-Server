...
#define PLAYER_RELEASE( pPlayerInfo )   if( pPlayerInfo ) { pPlayerInfo->lRefCount--; if( pPlayerInfo->lRefCount <= 0 ) DestroyPlayerStruct( pPlayerInfo ); } pPlayerInfo = NULL;
...
VOID     DestroyPlayerStruct( APP_PLAYER_INFO* pPlayerInfo );
VOID     AddPlayerStruct( APP_PLAYER_INFO* pPlayerInfo );
HRESULT  RefreshStatusBarText( DPNID idLocal );
                        ...
                        LPTSTR pCmdLine, INT nCmdShow )
{
    ...
    g_pSessionInfo = new CSessionInfo( g_pDPClient );
    ...
    if( g_bWasLobbyLaunched && g_pNetClientWizard->HaveConnectionSettingsFromLobby() )
    {
        ...
        else
        {
            ...
            _tcsncpy( g_strLocalPlayerName, g_pNetClientWizard->GetPlayerName(), MAX_PATH-1 );
    ...
    else
    {
        ...
        g_pNetClientWizard->SetDefaultPort( SIMPLECLIENTSERVER_PORT );
                                  ...
                                  WPARAM wParam, LPARAM lParam )
{
    switch( msg ) 
    {
        ...
        case WM_APP_UPDATE_STATS:
        {
            ...
            RefreshStatusBarText( g_dpnidLocalPlayer );
            break;
        }
        ...
        case WM_APP_DISPLAY_WAVE:
        {
            ...
            _sntprintf( szWaveMessage, MAX_PATH-1, TEXT("%hs just waved at you, %s!\r\n"), 
        ...
        case DPN_MSGID_RECEIVE:
        {
            ...
            switch( pMsg->dwType )
            {
                case GAME_MSGID_SET_ID:
                {
                    ...
                    if( pReceiveMsg->dwReceiveDataSize != sizeof(GAMEMSG_SET_ID) )
                        break;
                ...
                case GAME_MSGID_CREATE_PLAYER:
                {
                    ...
                    strncpy( pPlayerInfo->strPlayerName, pCreatePlayerMsg->strPlayerName, MAX_PLAYER_NAME-1 );
                ...
                {
                    // Validate incoming data
                    if( pReceiveMsg->dwReceiveDataSize != sizeof(GAMEMSG_WAVE) )
                        break;
...
HRESULT RefreshStatusBarText( DPNID idLocal )
{
    ...
    if( idLocal == NULL )
    {
        // If the  local dpnid is not set, this should mean the player
