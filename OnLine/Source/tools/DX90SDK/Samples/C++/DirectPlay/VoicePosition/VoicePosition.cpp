...
#define VOICEPOSITION_PORT      2514
                      ...
                      INT nCmdShow )
{
    ...
    else
    {
        ...
        g_pNetConnectWizard->SetDefaultPort( VOICEPOSITION_PORT );
                    ...
                    if( lplvcd->nmcd.dwDrawStage == CDDS_PREPAINT )
                    {
                        ...
                        if( pt.x != g_pPlayerLocal->pt.x || pt.y != g_pPlayerLocal->pt.y )
                        {
                            ...
                            if( dwCurTime - s_dwLastSendTime > 50 )
                            {
                                ...
                                {
                                    DXTRACE_ERR_MSGBOX( TEXT("SendLocalPosition"), g_hrDialog );        
...
HRESULT OnInitDialog( HWND hDlg )
{
    ...
    if( FAILED( hr = SendLocalPosition( hDlg ) ) ) 
        return DXTRACE_ERR_MSGBOX( TEXT("SendLocalPosition"), hr );        
...
HRESULT UpdateGrid( HWND hDlg )
{
    ...
    do
    {
        ...
        if( hr == DPNERR_BUFFERTOOSMALL )
        {
            ...
            // Verify allocation
            if( NULL == pPlayersNew )
                hr = DXTRACE_ERR_MSGBOX( TEXT("UpdateGrid"), E_OUTOFMEMORY );
            else
                g_pPlayers = pPlayersNew;
        }
    } 
    ...
    for( DWORD i = 0; i<dwNumPlayers; i++ )
    {
        ...
        if( pPlayerInfo->dpnidPlayer == g_LocalPlayerDPNID )
        {
            ...
            if( g_pDSListener ) 
            {
                ...
                {
                    DXTRACE_ERR_MSGBOX( TEXT("SetPosition"), hr ); 
    ...
    switch( dwMessageId )
    {
        ...
        case DPN_MSGID_RECEIVE:
        {
            ...
            if( pMsg->dwType == GAME_MSGID_PLAYERMOVED )
            {
                ...
                if( pReceiveMsg->dwReceiveDataSize != sizeof(GAMEMSG_PLAYERMOVED) )
                    break;
        ...
        case DPN_MSGID_HOST_MIGRATE:
        {
            ...
            {
                SetWindowText( g_hDlg, TEXT("VoicePosition (Session Host)") );
                g_bHostPlayer = TRUE;
            }
        ...
        case DVMSGID_CREATEVOICEPLAYER:
            {
                ...
                if( (DVID) pCreateVoicePlayerMsg->dvidPlayer != g_LocalPlayerDPNID )
                {
                    ...
                    {
                        LeaveCriticalSection( &pPlayerInfo->csPlayer );
                        return DXTRACE_ERR_MSGBOX( TEXT("Create3DSoundBuffer"), hr );    
                    }

                    if( FAILED( hr = pPlayerInfo->pDSBVoice->SetMinDistance( 5.0f, DS3D_DEFERRED ) ) )
                    {
                        LeaveCriticalSection( &pPlayerInfo->csPlayer );
                        return DXTRACE_ERR_MSGBOX( TEXT("SetMinDistance"), hr );        
                    }

                    if( FAILED( hr = pPlayerInfo->pDSBVoice->SetMaxDistance( 20.0f, DS3D_DEFERRED ) ) )
                    {
                        LeaveCriticalSection( &pPlayerInfo->csPlayer );
                        return DXTRACE_ERR_MSGBOX( TEXT("SetMaxDistance"), hr );       
                    }
                    ...
                    {
                        LeaveCriticalSection( &pPlayerInfo->csPlayer );
                        return DXTRACE_ERR_MSGBOX( TEXT("SetVelocity"), hr ); 
                    }

                    LeaveCriticalSection( &pPlayerInfo->csPlayer );

                    if( FAILED( hr = g_pDSListener->CommitDeferredSettings() ) )
                        return DXTRACE_ERR_MSGBOX( TEXT("CommitDeferredSettings"), hr );        
                }
        ...
        case DVMSGID_DELETEVOICEPLAYER:
        {
            ...
            if( pMsg->dvidPlayer != g_LocalPlayerDPNID )
            {
                ...
                if( FAILED( hr = pVoiceClient->Delete3DSoundBuffer( pPlayerInfo->dpnidPlayer,
                                                                    &pPlayerInfo->pDSBVoice ) ) )
                    DXTRACE_ERR_MSGBOX( TEXT("Delete3DSoundBuffer"), hr );        
            }
