...
// Global constants
//-----------------------------------------------------------------------------
const DWORD TUT08_VOICE_PORT = 2608; // The default port number
...
IDirectPlayVoiceClient*          g_pVoiceClient     = NULL;    // Voice client
IDirectPlayVoiceServer*          g_pVoiceServer     = NULL;    // Voice server
...
DWORD                            g_dwLocalPort      = TUT08_VOICE_PORT; // Host port
DWORD                            g_dwRemotePort     = TUT08_VOICE_PORT; // Connect port
...
WCHAR g_wstrAppName[] = L"Voice";
WCHAR g_wstrExecutable[] = L"Voice.exe";
                      ...
                      LPSTR pCmdLine, INT nCmdShow )
{
    ...
    if( g_hLobbyHandle != NULL )
    {
        ...
        {
            OutputError( TEXT("Failed to LobbyLaunch"), hr );
            goto LCleanup;
        }
    }
...
Break_ENUM_HOSTS_RESPONSE:
        ...
        case DPN_MSGID_HOST_MIGRATE:
        {
            ...
            // Reinitialize DirectPlayVoice since host has changed (which means
            // the voice server has moved too).
            // Init DirectVoice
            if( FAILED( hr = InitDirectPlayVoice() ) )
            {
                OutputError( TEXT("Failed Initializing DirectPlayVoice.")
                             TEXT("\n\nThe sample will now exit.") );
                EndDialog( g_hDlg, 0 );
            }
...
HRESULT WINAPI DirectVoiceClientMessageHandler(PVOID pvUserContext, DWORD dwMessageId, PVOID pMsgBuffer)
{
    HRESULT     hr = S_OK;
    HICON       hIconNew = NULL;
    static int  nPlayersTalking = 0;

    switch(dwMessageId)
    {
        case DVMSGID_RECORDSTART:
        {
            // Load and set active microphone icon
            hIconNew = LoadIcon( g_hInst, MAKEINTRESOURCE(IDI_MIC) );
            SendMessage( GetDlgItem( g_hDlg, IDC_MIC ), STM_SETICON,
                             (WPARAM) hIconNew, (LPARAM) 0 );
             break;
        }

        case DVMSGID_RECORDSTOP:
        {
            // Load and set inactive microphone icon
            hIconNew = LoadIcon( g_hInst, MAKEINTRESOURCE(IDI_MIC_DIS) );
            SendMessage( GetDlgItem( g_hDlg, IDC_MIC ), STM_SETICON,
                             (WPARAM) hIconNew, (LPARAM) 0 );
            break;
        }
        ...
        {
            if( nPlayersTalking == 0 )
            {
                // Load and set active speaker icon
                hIconNew = LoadIcon( g_hInst, MAKEINTRESOURCE(IDI_SPEAKER) );
                SendMessage( GetDlgItem( g_hDlg, IDC_SPEAKER ), STM_SETICON,
                             (WPARAM) hIconNew, (LPARAM) 0 );
            }
        ...
        case DVMSGID_PLAYERVOICESTOP:
        {
            ...
            if( nPlayersTalking == 0 )
            {
                // Load and set inactive speaker icon
                hIconNew = LoadIcon( g_hInst, MAKEINTRESOURCE(IDI_SPEAKER_DIS) );
                SendMessage( GetDlgItem( g_hDlg, IDC_SPEAKER ), STM_SETICON,
                             (WPARAM) hIconNew, (LPARAM) 0 );
            }
            break;
...
HRESULT InitDirectPlayVoice()
{
    ...
    if( g_bHosting )
    {
        ...
        {
            OutputError( TEXT("Failed Creating the IDirectPlayVoiceServer Object"), hr );
            return hr;
        }
        ...
        {
            OutputError( TEXT("Failed Initializing the IDirectPlayVoiceServer Object"), hr );
            return hr;
        }
        ...
        {
            OutputError( TEXT("Failed Starting the Session"), hr );
            return hr;
        }
    }
    ...
    {
        OutputError( TEXT("Failed Testing DirectVoice"), hr );
        return hr;
    }
    ...
    {
        OutputError( TEXT("Failed Creating the IDirectPlayVoiceClient Object"), hr );
        return hr;
    }
    ...
    {
        OutputError( TEXT("Failed Initializing the IDirectPlayVoiceClient Object"), hr );
        return hr;
    }
    ...
    {
        OutputError( TEXT("Failed DirectVoice Client Connect"), hr );
        return hr;
    }
    ...
    {
        OutputError( TEXT("Failed SetTransmitTargets"), hr );
        return hr;
    }
...
HRESULT TestDirectVoice()
{
    ...
    if( FAILED( hr = CoCreateInstance(CLSID_DirectPlayVoiceTest, NULL, 
                                    ...
                                    (LPVOID*) &pVoiceTest ) ) )
    {
        OutputError( TEXT("Failed Creating the IDirectPlayVoiceTest Object"), hr );
        goto LCleanup;
    }
    ...
    {
        if( FAILED( hr = pVoiceTest->CheckAudioSetup(&guidPlayback, &guidCapture, g_hDlg, DVFLAGS_ALLOWBACK ) ) )
        {
            OutputError( TEXT("Failed CheckAudioSetup"), hr );
            goto LCleanup;
        }
    }
    else if( FAILED( hr))
