...
// File: Connect.cpp
//
// Desc: This tutorials adds the ability to connect to an existing session.
//
...
// Global constants
//-----------------------------------------------------------------------------
const DWORD TUT04_CONNECT_PORT = 2604; // The default port number
...
DWORD                g_dwLocalPort     = TUT04_CONNECT_PORT; // Host port
DWORD                g_dwRemotePort    = TUT04_CONNECT_PORT; // Connect port
                ...
                case IDC_SESSIONS:
                    switch( HIWORD(wParam) )
                    {
                        case LBN_SELCHANGE:
                            ...
                            EnableWindow( GetDlgItem( g_hDlg, IDC_CONNECT ), ( LB_ERR !=
                            SendMessage( GetDlgItem( g_hDlg, IDC_SESSIONS ), LB_GETCURSEL, 0, 0 ) ) );
                            return TRUE;
                    }
                    break;
...
HRESULT ConnectToSession()
{
    ...
    // Determine which host the user has selected
    iSelHost = SendMessage( GetDlgItem( g_hDlg, IDC_SESSIONS ), LB_GETCURSEL, 0, 0 );
    if( iSelHost == LB_ERR )
        return S_OK;
...
void UpdateUI()
{
    ...
    if( g_bHosting || g_bConnected )
    {
        ...
        SendMessage( GetDlgItem( g_hDlg, IDC_SESSIONS ), LB_ADDSTRING, 0,
                     (LPARAM) TEXT("Host enumeration disabled while connected.") );   
    }
    else
    {
        SendMessage( GetDlgItem( g_hDlg, IDC_STATUS ), WM_SETTEXT, 0, 
    ...
    EnableWindow( GetDlgItem( g_hDlg, IDC_CONNECT ), FALSE );
    EnableWindow( GetDlgItem( g_hDlg, IDC_ADDRESS ), !(g_bHosting | g_bConnected) );
    EnableWindow( GetDlgItem( g_hDlg, IDC_REMOTE_PORT ), !(g_bHosting | g_bConnected) );
    EnableWindow( GetDlgItem( g_hDlg, IDC_SEARCH ), !(g_bHosting | g_bConnected) );
}
