...
DPNID              g_HostPlayerDPNID;                       // DPNID of host player
                      ...
                      INT nCmdShow )
{
    ...
    else
    {
        ...
        g_pNetConnectWizard->SetDefaultPort( VOICECONNECT_PORT );
    ...
    if( bConnectSuccess )
    {
        ...
        DialogBox( hInst, MAKEINTRESOURCE(IDD_VOICE_SETUP), NULL, (DLGPROC) VoiceConfigDlgProc );
    }
...
HRESULT DisplayPlayersInChat( HWND hDlg )
{
    ...
    for( DWORD i = 0; i<dwNumPlayers; i++ )
    {
        ...
        lvItem.iSubItem   = 1;
        lvItem.pszText    = strStatus;
        lvItem.cchTextMax = (int) _tcslen( strStatus );
                                         ...
                                         PVOID pMsgBuffer )
{
    ...
    switch( dwMessageId )
    {
        ...
        case DPN_MSGID_HOST_MIGRATE:
        {
            ...
            {
                SetWindowText( g_hDlg, TEXT("VoiceConnect (Session Host)") );
                g_bHostPlayer = TRUE;
            }
...
HRESULT VoiceConfigEnumCompressionCodecs( HWND hDlg )
{
    ...
    // For each type found, add an item to the listbox
