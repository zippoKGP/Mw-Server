...
BOOL                 g_abOutputFormatSupported[20];
                      ...
                      INT nCmdShow )
{
    ...
    else
    {
        ...
        {
            DXTRACE_ERR_MSGBOX( TEXT("CreateOutputBuffer"), hr );
...
HRESULT OnInitFormatsDialog( HWND hDlg )
{
    ...
    if( FAILED( hr = ScanAvailableOutputFormats() ) )
        return DXTRACE_ERR_MSGBOX( TEXT("ScanAvailableOutputFormats"), hr );
...
HRESULT OnOutputFormatBoxSelected( HWND hDlg )
{
    ...
    if( FAILED( hr = ScanAvailableInputFormats() ) )
        return DXTRACE_ERR_MSGBOX( TEXT("ScanAvailableInputFormats"), hr );
...
HRESULT OnFormatsOK( HWND hDlg )
{
    ...
    if( FAILED( hr = SetBufferFormats( &wfxInput, &wfxOutput ) ) )
        return DXTRACE_ERR_MSGBOX( TEXT("SetBufferFormats"), hr );
            ...
            switch( LOWORD(wParam) )
            {
                ...
                case IDC_RECORD:
                    ...
                    if( g_bRecording )
                    {
                        ...
                        {
                            DXTRACE_ERR_MSGBOX( TEXT("StartBuffers"), hr );
...
HRESULT StartBuffers()
{
    ...
    g_pDSBOutput->SetCurrentPosition( g_dwNextOutputOffset );
