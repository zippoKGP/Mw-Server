            ...
            case IDC_PLAY:
                {
                    ...
                    {
                        DXTRACE_ERR_MSGBOX( TEXT("PlayBuffer"), hr );
...
VOID LoadWaveAndCreateBuffer( HWND hDlg, TCHAR* strFileName )
{
                                                  ...
                                                  dwNotifySize, g_hNotificationEvent ) ) )
    {
        if( hr != DSERR_BADFORMAT && hr != E_INVALIDARG )
            DXTRACE_ERR_MSGBOX( TEXT("CreateStreaming"), hr );

        SetDlgItemText( hDlg, IDC_FILENAME, TEXT("Could not support the file's audio format.") );
        return;
    }
...
HRESULT PlayBuffer( BOOL bLooped )
{
    ...
    if( FAILED( hr = g_pStreamingSound->Reset() ) )
        return DXTRACE_ERR_MSGBOX( TEXT("Reset"), hr );
