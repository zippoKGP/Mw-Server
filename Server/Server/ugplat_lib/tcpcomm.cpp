...
UG_INT32 CTCPComm::getEncrypt(UG_PVOID pvConnect,UG_DWORD& dwEncrypt)
{
	if(m_pServerComm)
	{
		return ((CTCPServer*)m_pServerComm)->getEncrypt(pvConnect,dwEncrypt);
	}
	return -1;
}


UG_INT32 CTCPComm::getEncrypt(UG_DWORD& dwEncrypt)
{
	if(m_pClientComm)
	{
		return ((CTCPClient*)m_pClientComm)->getEncrypt(dwEncrypt);
	}
	return -1;
}
