...
UG_ULONG CUGMySql::getQueryResult(UG_PVOID pvResult,UG_ULONG ulRowIndex,UG_ULONG ulFieldIndex,UG_PCHAR& pchResult)
{
	...
	if(m_pMySql)
	{
		if(pResult)
		{
			...
			mysql_row_seek(pResult,pRow);
			mysql_fetch_row(pResult);
			LPDWORD p = mysql_fetch_lengths(pResult);
			pchResult = *(pRow->data + ulFieldIndex);
			if(p)
			{
				return *(p + ulFieldIndex);
			}
			return 0;
		}
		return -1;
...
UG_ULONG CUGMySql::convertString(UG_PCHAR pchNew,UG_PCHAR pchOld)
{
	if(!pchOld)
	{
		return 0;
	}
	return convertString(pchNew,pchOld,strlen(pchOld));
}

UG_ULONG CUGMySql::convertString(UG_PCHAR pchNew,UG_PCHAR pchOld,UG_INT32 nOldLen)
{
	if(m_pMySql)
	{
		if(!pchOld && !pchNew)
		...
		UG_ULONG ulLen = mysql_real_escape_string(m_pMySql,pchNew,pchOld,nOldLen);
		*(pchNew + ulLen) = '\0';
		return ulLen;
	}
	return 0;
