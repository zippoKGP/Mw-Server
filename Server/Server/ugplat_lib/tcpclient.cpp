...
UG_ULONG CTCPClient::init(PINIT_CLIENT pInit)
{
	...
	char szLog[1024];
	sprintf(szLog,"%d.log",GetTickCount());
//	m_log.init(szLog);
	...
	else
	{
		...
		for(UG_INT32 i = 0; ; i ++)
		{
			...
			if(m_nRecvPos > ulBufLen) //数据大
			{
				...
				m_log.UGLog("CTCPClient ip = %s, port = %x, id = %d, param = %d.",convertip(m_pInit->dwIP),m_pInit->dwPort,tp.ugGcp.ulSvrType,tp.ugGcp.ulSvrParam);
				...
				else if(NET_MSG_CHECK_ONLINE == tp.ugGcp.ulSvrType)
				{
					...
					ut.ulSvrParam = m_dwRecvAllDataSize;
					ut.wDataLen = 0;
					sendData(&ut,NULL);
					m_log.UGLog("NET_MSG_CHECK_ONLINE CTCPClient ip = %s, port = %x, id = %d, param = %d.",convertip(m_pInit->dwIP),m_pInit->dwPort,tp.ugGcp.ulSvrType,tp.ugGcp.ulSvrParam);
				}
				else
				{
...
UG_INT32 CTCPClient::getEncrypt(UG_DWORD& dwEncrypt)
{
	dwEncrypt = m_ulCheckDataOver;
	return 0;
}
