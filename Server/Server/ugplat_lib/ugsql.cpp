...
UG_ULONG CUGSQL::convertString(UG_PCHAR pchNew,UG_PCHAR pchOld)
{
	CUGMySql* pSql = (CUGMySql*)m_pMySql;
	if(pSql)
	{
		return pSql->convertString(pchNew,pchOld);
	}
	return 0;
}

UG_ULONG CUGSQL::convertString(UG_PCHAR pchNew,UG_PCHAR pchOld,UG_INT32 nOldLen)
{
	CUGMySql* pSql = (CUGMySql*)m_pMySql;
	if(pSql)
	{
		return pSql->convertString(pchNew,pchOld,nOldLen);
	}
	return 0;
}
