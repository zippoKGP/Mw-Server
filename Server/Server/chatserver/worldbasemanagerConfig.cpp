...
UG_LONG CWorldbaseManagerConfig::init(UG_PCHAR pchPathName)
{
	...
	if(ulRet)
	{
		printf("init ini file error, file = %s.\n",pchPathName);
		return -1;
	}
	ulRet = ini.getValue("WorldbaseManager","IPForWorldbase",pchData);
	if(!pchData)
	{
		printf("getValue IPForWorldbase from ini file.\n");
		return -1;
	}
	...
	if(ulRet)
	{
		printf("getValue WorldbaseManager from ini file.\n");
		return -1;
	}
	m_lWorldbaseCount = ini.getCount("WorldbaseManager") - 2;
	if(m_lWorldbaseCount < 1)
	{
		printf("getCount error from ini file, count =  %d.\n",m_lWorldbaseCount);
		return -1;
	}
	...
	for(UG_LONG l = 0; l < m_lWorldbaseCount + 2; l ++)
	{
		...
		{
			printf("init the %d index chat config failed.\n",i);
			ReleasePA<CWorldbaseConfig*>(m_pWorldbaseConfig);
			return -1;
		}
		i ++;
