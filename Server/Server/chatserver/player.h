...
public:
	...
	UG_LONG			sendUnionChannel(UG_INT32 n32id[],UG_INT32 nCount,UG_INT32 n32Pos);
	UG_LONG			sendWorldChannel(UGINT nCheck);
...
public:
	UG_LONG			cout(FILE* p);
	UG_LONG			sendData(UG_DWORD dwMsgID,UG_DWORD dwParam,UG_PCHAR p,UG_ULONG ulLen,DWORD dwOverflow = 0);
	UG_LONG			sendGMChannel(GMMSG* p);
	
protected:
	UG_LONG			filterFriend(UG_INT32 n32Playerid);
	UG_LONG			filterTrade(UGPCHAR pchContent);
	UG_LONG			setCommandConfig(UG_PCHAR pchData,UG_ULONG ulLen);
	UGBOOL			isInFriendList(UG_INT32 n32Playerid);
...
protected:
	...
	UG_LONG			sendUnionChannel(UG_INT32 n32Playerid,UG_PCHAR p,UG_ULONG ulLen);
	...
	UG_LONG			sendGMChannel(UG_PCHAR p,UG_ULONG ulLen);

protected:
	UGLONG			filterStopTalk();
	UGLONG			sendGMMsg(UGINT nChannel,UGINT nPlayerid,UGPCHAR pchNickName,UGPCHAR pchContent);
	
protected:
	UGLONG			logGM(UGINT nType,UGINT nID,UGPCHAR pchNickName,UGINT nNLen,UGPCHAR pchContent,UGINT nCLen);
	UGBOOL			isBeGMLog();
	UGBOOL			filterSymbol();
...
public:
	UG_PCHAR		m_pchWorldChannelBuffer;//世界频道数据缓冲
	UG_LONG			m_lWorldChannelBuffer; //世界频道数据缓冲大小

public:
	UGPCHAR			m_pchUnionBuffer; //(10 * (MAX_CHAT_LENGTH + 1))BYTE
	UGLONG			m_lUnionBufferPos;
	UGINT			m_nUnionPos;
